...
In ``went there yesterday'', the advmod ``there'' is annotated as a pArticipant,
while the oblique ``yesterday'' as an aDverbial.

\paragraph{amod.}
Adjectives are annotated as aDverbials, Elaborators or States,
depending on whether they modify scene or non-scene nouns
(``romantic dinner'' vs. ``medical school''), or
they are evoking scenes on their own (``beautiful hotel'').

\paragraph{compound.}
While most compounds correspond to Elaborators (e.g., ``industry standard''),
many are unanalyzable expressions (e.g., ``figure out'').
However, this is not a semantic category.
For example, in many cases a compound represent an argument,
and is annotated as pArticipants (``food craving'').
