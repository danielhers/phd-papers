...
\begin{description}
  ...
  \item[csubj, dep and orphan.] Appear less than 10 times, and thus too rare to reach
...
\begin{description}
	...
	Annotated as pArticipant (e.g., ``\textit{Wine} was excellent'')
	or Process/State
	(e.g., ``But \textit{service} is very poor''),
	depending on whether the unit is a scene.
	
	\item[amod and nmod.]
	Adjectival modifiers are annotated as aDverbials or Elaborators,
	depending on whether they modify scene or non-scene phrases, respectively
	...
	However, it may be the other way around if they modify a scene
	(e.g., ``Great job \textit{master-keying our building}'').
	
	\item[cc, conj and list.]
	\verb|cc| is split between coNnector (between non-scenes) and Linker (between scenes),
	while \verb|conj| and \verb|list| are split between Center and H (parallel scene), due to the
...
\subsection{Arguments/Adjuncts}\label{sec:arguments}
      ...
      UCCA distinguishes between pArticipants (including locations and abstract entities), and secondary relations (aDverbials), 
      which cover manner, aspect and modality. Secondary relations can be verbs (e.g., ``begin'', ``fail''),
...
\begin{description}
	\item[det, expl, iobj, nsubj, and obj.]
