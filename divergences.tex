...
\subsection{Flattening of scene structure.}
...
det, iobj, nsubj: match almost one-to-one. no need for discussion (maybe a brief comment on determiner)

maybe also obj and expl are cases of almost one-to-one, but need to verify

aux and cc. aux: split between F (auxilliary verbs) and D (modals). TUPA gets ~100\%; same with cc: split between N and L (mention that it's a convention difference that we fixed)

mark: Split between R, L and F. A matter of convention. TUPA does perfectly on this.
