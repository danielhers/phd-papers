...
\title{Identifying Content Differences between Syntactic and Semantic Parsing}
...
\begin{abstract}
  Syntax can be used to inform semantic parsers,
  as shown by various methods such as multi-task learning and syntactic features.
  However, these methods are almost invariably generic,
  and do not assume any scheme-specific, language-specific, or
  ...
  (2) UCCA's distinction between particiapnts and secondary relations, not accounted for by UD; 
  (3) Differences in the treatment of inter-clause/inter-Scene linkage.
  ...
  To demonstrate the feasibility of taylored protocols for using syntax in semantic parsing,
  we show that a simple UD to UCCA coverter can be used to imrpove UCCA parsing performance.
\end{abstract}
...
One of the fundamental functions of syntax is to help semantics.
