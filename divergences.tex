...
The exceptions are cases of unanalyzable multi-word expressions,
which are analyzed in UD: ``a bit'', ``bites the dust'', ``when it comes to''.
In addition, nsubj and obj are annotated as process (P) or state (S) when they
represent a (nominal) predicate,
and the determiner (det) ``no'' as an adverbial (D) when it modifies one.
Some nsubj instances of ``it'' and ``that''
are analyzed as expletives in UCCA (and annotated as F),
and some wh-pronouns as linkers (L) or relators (R).
