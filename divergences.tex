...
\begin{description}
	...
	and many are unanalyzable expressions (e.g., ``\textit{figure out}'').
	...
	``\textit{a} bit'', ``bites \textit{the dust}'', ``when \textit{it} comes to'').
	\item[nummod.]
...
\begin{description}
	...
	both advcl and parataxis almost exactly correspond to Linked Scene.
...
\begin{description}
	\item[expl and nsubj.]
	Expletives are Function in UCCA.
	...
	\item[discourse.]
	Interjections and discourse markers are Linked Scene, Ground or Linker,
	...
	in UCCA a named entity will be the Center if it is one of the phrases.
\end{description}
