...
\section{Introduction}\label{sec:introduction}
  ...
  Nevertheless, despite the centrality of the use of syntax for semantics, 
	few works systematically assessed what content is encoded in semantic structures that is
  not encoded in syntactic ones, and vice versa. 
	Understanding such content differences will enable better use of the overlapping content between
	semantic and syntactic parsing, for their mutual benefit, as well as point at semantic distinctions that
	are unlikely to be resolved by syntax.
	Comparing syntactic and semantic treebanks can also support fine-grained error analysis of semantic parsers, as proposed
	by \citet{szubert2018structured} in the context of AMR parsing.
  ...
  as a test case. We select these two schemes due to their conceptual similarity, which
	allows us to focus on principle differences between the schemes, and largely ignore differences
	that stem from annotation convenetions. For instance, UD prefers lexical heads over functional ones. 
	We view this preference largely as a notational variant (since converting between the two can
	generally be done deterministically), but it does facilitate comparison as it 
	results in structures that resemble semantic schemes.
  ...
  \begin{enumerate}
    \item 
      UCCA distinguishes between Scenes (units that describe an event, state or activity) and non-Scene units, whereas
      UD does not. This distinction cuts across UD categories, as a Scene correspond to a noun phrase (``Mike's kick'')
			or a clause (``Mike kicks''). Put differently, UCCA distinguishes between eventive and relational nouns,
			and other types of nouns, whereas UD does not.
    \item
      UCCA distinguishes between participants (including locations and abstract entities), and secondary verbs, 
			expressing such distinctions as manner, aspect or modality. Secondary relations can corresponding to various
			syntactic units, including verbs, such as ``begin'' or ``fail'', prepositional phrases such as ``with disrespect'', 
			as well as adjectives or adverbs. 
    \item
			UD and UCCA have different treatment of multi-word expressions (MWEs). MWEs in UD include names, and fixed expressions
			that lack a clear syntactic structure (e.g., ``as well as''). UCCA takes a more inclusive approach, treating a variety
			of semantically opaque constructions as units (e.g., light verbs and idioms).
		\item
			UD categorizes clause linkage into coordination, subordination (e.g., adverbial clauses), argumenthood (complement clauses),
			and parataxis (a residual category). UCCA only distinguishes between argumenthood (where one Scene is
			a participant in another) and largely conflates clausal coordination, subordination and parataxis. 
...
\subsection{Similar Treatment}\label{sec:similar}
