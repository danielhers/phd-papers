...
Figure~\ref{fig:original_example_ud} shows an example UD tree, and Table \ref{} gives a short description of each UD category.
...
UD but not in UCCA (\S\ref{}).
Another source of unmatched units is inter-scene linkage, which tends to be flatter in
UCCA (see \S\ref{sec:linkage}).
Of the matched heads, about 70\%\oa{is this true?} correspond to UCCA Processes, States, Linked Scenes or Centers,
...
\begin{description}
    ...
    this corresponds to a flat structure: ``[they]$_A$ [have to]$_D$ pay$_P$''.\oa{we need to add abbreviations for the cateogries}
...
\begin{description}
    ...
    \item[Complex Prepositions.] Some complex prepositions, such as ``according to'' or ``on top of''\oa{do such things
    appear in our corpus? I know they appear in EWT}
...
\begin{description}
    ...
    (e.g., ``you \textit{can} graduate''). Semi-modals 
