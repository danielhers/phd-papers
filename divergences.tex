...
\begin{description}
	...
	Core syntactic dependents are usually semantic arguments,
	...
	(e.g., ``discount \textit{on services}'' or
	``Great job \textit{master-keying our building}'')
	or non-scene (e.g., ``\textit{my car's} gears and brakes'' or
...
\begin{description}
	...
	where the scene is flattened in UCCA
	(and so the complement clause is not a whole unit).
	\item[xcomp.]
	As they are dependents of verbs,
	in most cases, open clausal complements are pArticipant scenes
	(e.g., ``I asked them \textit{to change it}'').
	\item[vocative.]
