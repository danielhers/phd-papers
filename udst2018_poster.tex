...
\begin{center}
\large
   \begin{tikzpicture}[level distance=18mm, sibling distance=3cm]
   \node[anchor=west] at (0,6) {Parser state};
   \draw[color=gray,dashed] (0,-3.6) rectangle (15.5,5.45);
   \draw[color=gray] (.5,2.5) rectangle (6,4.5);
   \node[anchor=west] at (.5,3.3) {$S$};
   \node[fill=black, circle] at (3.2,3.5) {};
   \node[fill=purple, circle] at (4.7,3.5) {};
   \node[anchor=west] at (3.3,.775) {\scriptsize\ttfamily made};
   \draw[color=gray] (6.5,2.5) rectangle (15,4.5);
   \node[anchor=west] at (6.51,3.3) {$B$};
   \node[anchor=west] at (8.11,3.26) {\ttfamily to feel\ldots};
   \node[anchor=west] at (2.1,.8) {$G$};
   \node[fill=black, circle] at (8.35,.75) {}   
   ...
   \end{tikzpicture}
   \begin{tikzpicture}[->]
   \node[anchor=west] at (0,10) {Classifier};
   ...
   \foreach \i/\word in {0/{We},5/{were},10/{welcome},15/{.}} {
       \node (x\i) at (\i,1) {\ttfamily\word};
       \node[main] (e\i) at (\i,2.2) {};
       \path (x\i) edge (e\i);
       \path (e\i) edge (specific);
   }
    \node (x4) at (7,1) {\large\ldots};
    \node[main] (mlp) at (7,7.6) {MLP};
    \path (specific) edge (mlp);
    \coordinate (state) at (0,6.3);
    \path (state) edge [bend right] (mlp);
    \node (transition) at (7,9) {transition};
