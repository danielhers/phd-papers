...
These techniques have been used for tasks with varying degrees of similarity:
...
They are also used for domain adaptation \cite{W06-1615,P07-1033}.
%Multitask learning can be used as regularization, or to enlarge the effective training data.
Joint learning often replaces pipelines to alleviate cascading errors---for example,
joint tagging and parsing in transition-based systems \cite{bohnet2012transition}.

Representation learning has enabled much progress in multitask learning,
facilitating parameter sharing:
...
\oa{Mention shared task, mention that results using joint learning is not as good as pipeline
for parsing. Highlight practical aspect and difficulty thereof:
"The strength of newly developed neural methods has recently shown one of the first benefits
even from joint parsing over single-task parsing."
Even more general and challenging than multitask SDP, it is not clear that they will benefit each other.
SRL was built on top of syntax (args are mostly constituents), so it is clear that they will help.
For parsing - conducted in a restricted setting, fairly small gains.
Success of joint many-task in other fields.}
...
but we are not aware of a general transition-based DAG parser that handles AMR.
...
notably in the SemEval 2017 shared task \cite{may2017semeval}.\oa{say more about AMR parsing,
e.g., what apperoaches people have taken, and that parsers are taylored for it and why}
...
We consider one of the three formalisms used\oa{DM, PAS, PSD and CCD: aren't there four?} 
...
We also adopt the the standard evaluation for UCCA parsing is span-based,
...
with the highest score given by the classifier.\oa{I don't understand what you wrote after the comma. You mean it's greedy?}
...
we use feature embeddings representing the lemma, prefix, suffix, shape, and named entity type,\oa{what is prefix/suffix/shape? please write down the added feature templates}
all extracted using spaCy \cite{spacy2}.\footnote{\url{https://spacy.io}}
...
available at $<$anonymized$>$ {\bf it shouldn't be anonymized. we should put them on the HUJI NLP github and put a link here}}
...
Table~\ref{tab:corpora} shows the size of each corpus.\oa{do these corpora have standard splits? if so, say that. say you tune on dev w/o cross-validation}
...
Edges in predicted and gold graphs are matched by terminal yield and label.\oa{I think it's worth repeating in brief how these are defined}
...
WeSince training sets differ in size, we sample this many sentences from each training set.

All embeddings are initialized randomly.
For words, we additionally use external word embeddings containing 250K word vectors\footnote{you mean 250K dimensions?} from fastText
\cite{bojanowski2016enriching},\footnote{\url{https://github.com/facebookresearch/fastText}}
pre-trained over Wikipedia and updated during training.\oa{what do you mean ``additionally''? the word embeddings are much larger for English then?}
