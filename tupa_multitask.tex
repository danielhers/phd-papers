...
and of  3.9\% in an out-of-domain setting in
...
e.g., joint classification for different arguments in semantic role labeling \cite{toutanova2005joint},
%to joint parsing and named entity recognition \cite{Finkel2009JointPA},
and incorporation of unlabeled data for various tasks \cite{ando2005framework}.
...
Neural MTL has been mostly effective with formally similar
...
Another work closely related to ours is by \citet{P17-1186},
reporting benefits from MTL over three broad-coverage semantic dependency parsing (SDP)
tasks using different representations, albeit on the same text and using a uniform formalism with 
...
In this work, we take the challenge of MTL of semantics a step further,
improving UCCA parsing using multitask 
...
\caption{\label{fig:original_examples}
 Example graph for each tackled task.
...
We focus on representations providing full-sentence analyses,
...
Figure~\ref{fig:original_examples} presents one sentence annotated in all schemes.
...
but using only the average $F_1$ score on the UCCA development set as the criterion for early stopping.
...
We also use word, tag and dependency relation dropout \cite{kiperwasser2016simple}.\footnote{In
...
Table~\ref{tab:ood_results} shows main our experimental results.
...
The best MTL models are significantly
better than single-task (bootstrap test, $p<0.05$),
...
(observed on the out-of-domain English set too):
despite the \textit{enhanced++} graphs,
...
even coming from different domains.
...
To quantify the domain differences between the English training sets we used
and the out-of-domain test set (\textit{20K}),
we measure the L1 distance between the word unigram distributions in these corpora
\cite{Plank2011EffectiveMO},
given in Table~\ref{tab:domain_sim}.
All auxiliary training sets are more similar to the \textit{20K Leagues} set than \textit{Wiki}
...
out-of-domain setting, and their combination did on remote edges.
...
We demonstrated that MTL improves UCCA parsing in out-of-domain settings or when training data is scarce,
