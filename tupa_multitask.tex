...
To quantify the domain differences,
we measure the L1 distance between word distributions
\cite{Plank2011EffectiveMO}
in the English training sets and \textit{20K} test set
(Table~\ref{tab:domain_sim}).
For similarity between schemes,
we use unlabeled UCCA evaluation (ignoring edge labels)
on 100 English sentences from
Section 02 of the Penn Treebank Wall Street Journal (WSJ),
after conversion to the unified DAG format
(Table~\ref{tab:common})\footnote{The annotated
sentences are available at $<$anonymized$>$}.
While the benefit we showed for MTL is likely partly due to the variety of training domains,
the interaction with graph structure is less clear.
Due to the deep model architecture (\S\ref{sec:multitask}),
