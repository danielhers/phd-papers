...
%difference between domains \cite{Plank2011EffectiveMO}
...
The English part is used only as an out-of-domain test set.
For the French and German datasets we use a train/dev/test split of 413/67/67 and 4196/561/2174 sentences,
respectively.\footnote{\mbox{\url{http://github.com/huji-nlp/ucca-corpus}}}
...
See Supplementary Material for a listing of hyperparameter settings.
...
\small UD & 68.2$\star$ & 67.4$\star$ & 67.8$\star$ & 33.3$\star$ & 15.1$\star$ & 20.8$\star$ \\
\hline
\multicolumn{4}{l|}{\small \bf German (in-domain)} & \\
\small Single & 73.3 & 71.7 & 72.5 & 57.1 & 17.7 & 27.1 \\
\small UD & 73.7$\star$ & 72.6$\star$ & 73.2$\star$ & 61.8 & 24.9$\star$ & 35.5$\star$
\end{tabular}
\caption{
Experimental results, in percents, on the \textit{20K Leagues} set.
...
\end{tabular}
\caption{
Experimental results, in percents, on the English \textit{Wiki} test set.
...
In our case, none of the auxiliary tasks are annotated on the same corpus as our main task.
Furthermore, while UCCA is annotated over Wikipedia and a literary corpus,
...
we therefore found that the combination of many different domains benefits domain
adaptation even without training on the target domain.
...
on 100 manually annotated English sentences from the
