...
on UCCA parsing over single-task training in (1) an out-of-domain setting in English; 
(2) an in-domain setting in German; and (3) a setting in French, where training data is scarce.\footnote{Our code is publicly available at $<$anonymized$>$.}
...
\caption{\label{fig:original_examples}
 ...
 Figure \ref{fig:original_example_sdp} presents a DM semantic dependency graph,
  ...
  Punctuation tokens are excluded {\color{red} from SDP graphs}.
...
syntactic  annotation in a large variety of languages,
...
Inference is performed greedily,\oa{should we mention Kipperwasser and Goldberg here or not?}
...
The converted graph is actually a tree, as 
{\color{red} are all converted UD graphs. In UD enchanced++, the converted graphs need not form a tree.}
}
...
the core arguments defined for the frame\oa{as children?}.
...
increasing generalization.\oa{didn't you have a citation for this?}
...
training the parser in a single-task and multitask settings,
...
The English and French parts of 20K (v1.0) are a parallel corpus comprising the first five
chapters of the book.
As in previous work \cite{hershcovich2017a}, we use the English part only as an out-of-domain test set.
Additionally, we train and test on the small French part using the standard split,
as well as the German part (v0.9),
...
For AMR, DM and UD, we only use the training sets from standard train/dev/test splits.
...
Significance testing of improvements over the single-task model are performed 
using the bootstrap test \cite{berg2012empirical}, with $p<0.05$.
...
%: AMR or DM, or both, as auxiliaries.
...
%All auxiliary training sets are more similar to 20K than Wiki
...
\end{tabular}
\caption{Unlabeled F-scores between the representations of the same English sentences (from PTB WSJ), converted to the unified DAG format.
\label{tab:common}}
\end{table}
...
Task overlap\oa{what is task overlap? addressing the same task? is that MTL then?} is an important factor in MTL success
...
As a measure of the formal similarity of the different schemes sto UCCA,
