...
joint and multi-task learning\oa{we need to explain what the difference between them is or say we use one or ther other;
to me it seems that joint means any training of several tasks that informs one another, while
multi-task learning is mostly used for neural approaches that share some of the networks across tasks. is 
that right?} has been used for tasks with varying degrees of similarity:
...
I think this is just a type of semi-supervised learning.. are there two separate predictions here?}.
...
Such techniques effectively increase the amount of training data (exploiting 
incidental\oa{what is incidental supervision?} supervision \cite{DBLP:conf/aaai/Roth17}),
...
as scaffolding.
Examples of deep multi-task learning across similar tasks include 
multilingual syntactic \cite{Q16-1031,guo2016exploiting}\oa{is this dep parsing? if so, say that}
and semantic parsing \cite{}, as well as domain adaptation in semantic parsing
\cite{herzig-berant:2017:Short,W17-2607,duong2017multilingual}.\oa{split the references between the two cites}
...
one of the first works ever to report benefits from 
applying joint syntactic-semantic parsing (over the pipeline model) \citep{swayamdipta2017frame}.
...
Another work which is closely related to ours is \citep{P17-1186},
which reported benefits from multi-task learning over three\oa{is that right?} broad-coverage semantic dependency (SDP)
parsing tasks that use three different annotation schemes, albeit on the same text and using a uniform formalism with 
considerable overlap in their edges.
They report improvement to all tasks, of ...\% to ...\% increase in attachment accuracy.\oa{is that right?}

In this work we take the challenge of multi-task learning of semantics a step further and 
show benefit to UCCA parsing from multi-task 
training with conceptually and formally different formalisms (AMR and SDP; see Section \ref{}).
...
transition-based parsers have since been generalized to parse into many other graph
...
and named entity type,\daniel{Omri, what do you mean by "please write down the added feature templates"? Omri: put in
the feature templates you added in a format similar to what you did in the TUPA paper.}
...
- is it clear now? Omri: you mean you take the 250K most frequent ones? do you concatenate them with the 
...
Omri: I mean saying how the F-score measure is defined more explicitly.
}
