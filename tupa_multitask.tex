  ...
  \begin{tikzpicture}[level distance=15mm, ->,
    ...
    \node (ROOT) [fill=black,circle] {}
      child {node (after) [fill=black,circle] {}
      {
        ...
        child {node [word] {\quad graduation} edge from parent node {head}}
      } edge from parent node {obl}}
      child {node {}
      {
        child {node [word] (comma) {\quad\quad,{\color{white}g}} edge from parent [draw=none]}
      } edge from parent [draw=none]}
      child {node {}
      {
...
\citet{Finkel2009JointPA} showed that the combination of many different domains helps with domain
adaptation without having to identify the target domain.
