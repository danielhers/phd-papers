...
The English part of \textit{20K Leagues} is used only as an out-of-domain test set, as in HAR17.
For the French and German parts we use train/dev/test splits of 413/67/67 and 4196/561/2174 sentences,
respectively.\footnote{\mbox{\url{http://github.com/huji-nlp/ucca-corpus}}}
For each language, we tune on the development set without cross-validation.
...
While UCCA is annotated over Wikipedia and a literary corpus,
...
or noise (German), training on the \textit{20K Leagues} training set and evaluating on the test set.
...
Fur completeness, we also evaluate the English models on the \textit{Wiki} test set.
