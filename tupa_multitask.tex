...
To apply our parser to the four tasks presented in \S\ref{sec:tasks},
...
Additionally, we train and test on the fairly small French part using the standard split,
...
For AMR, DM and UD, we use only the training sets from standard train/dev/test splits.
...
Fur completeness, we also evaluate the English models on the Wiki in-domain test set.
...
for in-domain parsing (both training and test sets are from 20K):
13.8\% error reduction in French, having less than 10\% the amount of training data as English,
and 1\% in German, where the training set size is similar to the English Wiki one,
