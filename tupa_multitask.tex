...
They have been used in NLP with tasks of varying degrees of similarity,
as regularization, or to enlarge the training data:
...
In addition, UCCA and AMR contain non-terminal nodes.\oa{what about empty nodes?}

In order to parse graphs with these structural properties,
we extend TUPA \cite{hershcovich2017a}, originally developed for UCCA parsing,
...
whose terminals are (partially) aligned to text tokens.
To support AMR parsing, we take advantage of existing alignments of the graphs with the text
...
will correspond to an implicit node.\oa{how frequent is this in our settings?}
...
In training, though, we can still use gold node labels for feature extraction.\oa{i don't understand: we extract
features from info that is not available during test? does that make sense?}
...
\centering
	\begin{tabular}{l|ccc|ccc||ccc|ccc}
		...
		\hline
		\multicolumn{6}{l}{\small Ours (multitask) [{\bf currently dev scores, and without ensemble}]} &&&& \\
		AMR
		& 73.7 & 73.1 & 73.4 & 46.2 & 49.8 & 48 &&&& \\
		DM
		& 73.7 & 73.3 & 73.5 & 44 & 57.9 & 50 &&&& \\
		UD$^{++}$
		& 74.6 & 73.7 & 74.2 & 61.7 & 20.6 & 30.9 &&&& \\
		AMR + DM
		& 74.1 & 74.2 & 74.1 & 49.4 & 50.5 & 49.9 &&&& \\
		AMR + UD$^{++}$
		& 73.5 & 73.4 & 73.4 & 51.3 & 42.4 & 46.4 &&&& \\
		DM + UD$^{++}$
		& 74.5 & 74.4 & 74.4 & 51.4 & 46.7 & 48.9 &&&& \\
		AMR + DM + UD$^{++}$
		& 72.6 & 74.9 & 73.7 & 49.8 & 46.4 & 48.1 &&&& \\
		\multicolumn{6}{l}{\small Ours (ensemble of models trained with different auxiliary tasks)} &&&& \\
		AMR + DM & \\
		AMR + UD$^{++}$ & \\
		DM + UD$^{++}$ & \\
		AMR + DM + UD$^{++}$ &
