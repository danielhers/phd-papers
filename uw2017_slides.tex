...
\date{University of Washington \\ July 26, 2017}

\begin{frame}
\titlepage
\end{frame}
\note{Do not say paper's title. \\
Say "hi, I'm Daniel and I'll tell you about a pretty general semantic parser".}
...
\begin{center}
  \begin{tikzpicture}[level distance=12mm, sibling distance=2cm, ->,
    ...
    \node (ROOT) [fill=blue,circle] {}
      child {node (You) [draw=black,circle,inner sep=1pt] {1} edge from parent}
      child {node [draw=black,circle,inner sep=1pt] {2} edge from parent}
      child {node (totakealongbath) [fill=blue,circle] {}
      {
        child {node [draw=black,circle,inner sep=1pt] {3} edge from parent}
        child {node (takeabath) [fill=blue,circle] {}
        {
          child {node [draw=black,circle,inner sep=1pt] {4} edge from parent}
          child {node [draw=black,circle,inner sep=1pt] {5} edge from parent}
          child {node (long) [draw=black,circle,inner sep=1pt] {6} edge from parent[white]}
          child {node [draw=black,circle,inner sep=1pt] {7} edge from parent}
        } edge from parent}
      } edge from parent}
      ;
    \draw[bend left,dashed,->,orange,very thick] (takeabath) to node {} (You);
    \draw[bend left,->,red,very thick] (totakealongbath) to node {} (long);
  \end{tikzpicture}
\end{center}
\end{frame}
\note{"What I'll show you today is a parser that can generate output that support X, Y, and Z. This combination is not supported by any other parser. And we do need this combination, because semantic analysis schemes like AMR and UCCA need it." \\
"But before talking about semantic schemes, let's consider the classical approaches..."}
...
\begin{itemize}
	...
	\item Syntactic dependencies \cite{nivre2005dependency}
		\item Semantic dependencies \cite{oepen2016towards}
		\item AMR \cite{banarescu2013abstract}
		\item UCCA \cite{abend2013universal}
	\item Other semantic representation schemes\footnote{See recent survey \cite{abend2017the}}
	\vfill
		\begin{center}
...
\end{itemize}
\end{frame}
\note{Very brief introduction to semantic representation.
For a survey on this topic, see this paper from my lab, which is also in this year's ACL. \\
Semantic representation abstracts away from syntactic detail that does not affect meaning.}
...
\end{wraptable}
\end{frame}
\note{Some properties of UCCA that make it interesting to work on (not "advantages"). \\
...
Hebrew
\end{frame}
\note{Same guidelines across languages (not "definitions"). \\
A parallel English-French corpus has been annotated, and a German corpus is almost complete.}

\begin{frame}
\frametitle{Graph Structure}
UCCA generates a directed acyclic graph (DAG), where the text tokens are terminals.
Structural properties:
\begin{enumerate}
\item<1-> {\color{blue} Non-terminal nodes} --- hierarchy of entities and events
...
Parse text $w_1 \ldots w_n$ to graph $G=(V,E,\ell)$ incrementally.
...
\begin{flushleft}
\footnotesize
\textsc{Shift}, \textsc{Right-Edge\textsubscript A}, \textsc{Shift}, \textsc{Swap}, \textsc{Right-Edge\textsubscript P}, \textsc{Reduce}, \textsc{Shift}, \textsc{Shift}, \textsc{Node\textsubscript F}, \textsc{Reduce}, \textsc{Shift}, \textsc{Shift}, \textsc{Node\textsubscript C}, \textsc{Reduce}, \textsc{Shift}, \textsc{Right-Edge\textsubscript P}, \textsc{Shift}, \textsc{Right-Edge\textsubscript F}, \textsc{Reduce}, \textsc{Shift}, \textsc{Swap}, \textsc{Right-Edge\textsubscript D}, \textsc{Reduce}, \textsc{Swap}, \textsc{Right-Edge\textsubscript A}, \textsc{Reduce}, \textsc{Reduce}, \textsc{Shift}, \textsc{Shift}, \textsc{Left-Remote\textsubscript A}, \textsc{Shift}, \textsc{Right-Edge\textsubscript C}, \textsc{Finish}
\end{flushleft}
\end{frame}
...
\begin{center}
	...
	\depedge{5}{7}{A}
	\end{dependency}
	\captionof{figure}{Bilexical DAG approximation (for tree, delete remote edges).}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Future Work: UCCA}
Not yet handled by TUPA:
\begin{itemize}
	\item Linkage.
	\item Implicit units.
	\item Inter-sentence relations.
\end{itemize}
\begin{flushright}
...
\begin{frame}
\frametitle{Future Work: AMR}
Challenges:
\begin{itemize}
	\item Node labels.
	\item Partial alignment.
	\item Implicit nodes.
\end{itemize}
\begin{center}
  \begin{tikzpicture}[level distance=14mm, ->,
      every node/.append style={sloped,anchor=south,auto=false,font=\small},
    ...
    \node (ROOT) [fill=black, circle] {}
      child {node [fill=black, circle] {}
      {
	        child {node [word] {After} edge from parent node {}}
	        child {node (graduation) [fill=black, circle] {}
		    {
		      child {node [word] {graduation} edge from parent node {}}
		    } edge from parent node {\scriptsize op1} }
      } edge from parent node {\scriptsize time} }
      child {node (Joe) [fill=black, circle] {}
      {
      	child {node {}
	    {
        ...
        child {node [fill=black, circle] {}
	    {
		    child {node [fill=black, circle] {}
		    {
		      child {node [word] {Joe} edge from parent node {}}
		    } edge from parent node {\scriptsize op1} }
	    } edge from parent node {\scriptsize name} }
      } edge from parent node {\scriptsize ARG0} }
      child {node [fill=black, circle] {}
      {
        child {node {}
	    {
        ...
        child {node [fill=black, circle] {}
	    {
		    child {node [fill=black, circle] {}
		    {
		      child {node [word] {Paris} edge from parent node {}}
		    } edge from parent node {\scriptsize op1} }
	    } edge from parent node {\scriptsize name} }
      } edge from parent node {\scriptsize ARG2} }
      ;
      \draw[bend left] (ROOT) to node {} (moved);
      \draw[dashed] (graduation) to node {\scriptsize ARG0} (Joe);
  \end{tikzpicture}
...
\begin{itemize}
 ...
 \item Broad coverage of UCCA (linkage, implicit, inter-sentence).
