...
\end{enumerate}
--- properties needed for AMR, UCCA.
\begin{center}
  \begin{tikzpicture}[level distance=12mm, sibling distance=2cm, ->,
      every node/.append style={font=\rmfamily,text=white}]
...
\begin{itemize}
	\item Dependency grammar: bilexical tree, purely syntactic.
\end{itemize}

Semantic representation \cite{abend2017the}:
\begin{itemize}
	\item Semantic dependencies: bilexical graph, coupled with syntax.
	\only<2>{
	\item AMR \cite{banarescu2013abstract}, UCCA \cite{abend2013universal}: direct semantic representation. \\
...
\only<1>{
	...
	\begin{dependency}
		...
		\depedge[edge below,edge unit distance=1em,red]{1}{5}{ARG1}
		\depedge[edge below]{5}{4}{ARG1}
		\depedge[edge below,red]{6}{5}{ARG1}
