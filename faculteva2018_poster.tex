...
\begin{minipage}{.6\columnwidth}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \captionof{figure}{Universal Conceptual Cognitive Annotation ({\color{violet} UCCA}).}
  \end{center}
\end{minipage}
\hfill
\begin{minipage}{.2\columnwidth}
  \begin{center}
  \begin{adjustbox}{margin=2mm,frame,scale=.7}
  \color{violet}
  \begin{tabular}{cl}
	  P & process \\
	  S & state \\
  ...
  \end{tabular}
  \end{adjustbox}
  \captionof{table}{{\color{violet} UCCA} edge labels.}
  \end{center}
\end{minipage}
...
\begin{minipage}{.45\columnwidth}
  ...
  \end{tikzpicture}
  \captionof{figure}{Abstract Meaning Representation ({\color{teal} AMR}).}
  \end{center}
\end{minipage}
\begin{minipage}{.55\columnwidth}
    ...
    \end{dependency}
    \captionof{figure}{Semantic Dependencies ({\color{blue} SDP}): {\color{blue} DM} representation.}
    \vfill
    ...
    \end{dependency}
  \captionof{figure}{Universal Dependencies ({\color{purple} UD}).}
  \end{center}
\end{minipage}
...
\begin{itemize*}
\item [\textbf{\color{violet} UCCA}.] (1) \textbf{English} Wikipedia (\textbf{\color{violet} Wiki});
(2) \textit{Twenty Thousand Leagues Under the Sea} (\textbf{\color{violet} 20K}),
annotated in \textbf{English} (small, only test) \textbf{French} (small), and \textbf{German}
(pre-release, noisy).
\item [\textbf{\color{teal} AMR}.] LDC2017T10 (\textbf{English}).
\item [\textbf{\color{blue} SDP}.] DM part from SDP 2016 (\textbf{English}).
\item [\textbf{\color{purple} UD}.] \textbf{English}, \textbf{French} and \textbf{German} treebanks,
v2.1 (UD$^{++}$ for English).
\end{itemize*}
...
\pgfplotstableread[row sep=\\,col sep=&]{
	...
	\color{purple} \textbf{UD} & 17062 & 0 & 0 \\
	\color{blue} \textbf{SDP} & 33964 & 0 & 0 \\
	\color{teal} \textbf{AMR} & 36521 & 0 & 0 \\
	\color{violet} \textbf{Wiki} & 4268 & 454 & 503 \\
	\color{violet} \textbf{20K} & 0 & 0 & 506 \\
    }\english
\begin{center}
    \begin{tikzpicture}
    \begin{axis}[
...
For training, we use an oracle, which returns correct transitions given gold-standard graphs.
...
\begin{center}
   ...
   \node[anchor=west] at (-7,17) {\textbf{Classifier}};
   ...
   \node[main,color=Navy] (shared) at (22,10) {Shared BiLSTM};
   \node[color=Navy] (embeddings) at (10.5,4.5) {Shared embeddings};
   \foreach \i/\word in {-4/{After},4/{graduation},17/{to},25/{Paris}} {
       \node (x\i) at (\i,3) {\word};
       \node[main,color=Navy] (e\i) at (\i,6) {};
       \path[color=Navy] (x\i) edge (e\i);
       \path (e\i) edge (specific);
       \path[color=Navy] (e\i) edge (shared);
   }
    \node (x4) at (10.5,3) {\ldots};
    \node[main] (mlp) at (10.5,12) {Task-specific MLP};
    \path (specific) edge (mlp);
    \path[color=Navy] (shared) edge (mlp);
...
and generalizing a transition-based DAG parser.\footnote{Code:
