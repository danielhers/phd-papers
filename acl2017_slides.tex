...
Say "hi, I'm Daniel and I'll tell you about a general semantic parser".}
...
\only<1>{
	...
	\begin{dependency}
		...
		\depedge{7}{5}{det}
		\depedge{7}{6}{amod}
		\deproot[edge below,edge unit distance=1em]{2}{top}
		\depedge[edge below]{2}{4}{ARG2}
		\depedge[edge below,orange]{2}{1}{ARG1}
		\depedge[edge below,orange,edge unit distance=1.6em,edge start x offset=6pt]{4}{1}{ARG1}
		\depedge[edge below,orange]{4}{7}{ARG2}
		\depedge[edge below,orange]{5}{7}{BV}
		\depedge[edge below,orange]{6}{7}{ARG1}
	\end{dependency}
	\end{adjustbox}\\
	\vspace{-1cm}
	\begin{flushright}
...
Same guidelines across languages (not "definitions"). \\
...
The text tokens are terminals. \\
Allows {\color{blue} non-terminal nodes}, {\color{orange} reentrancy} and {\color{red} discontinuity}.
...
\hspace*{1cm}
  ...
  \tikzstyle{word} = [font=\rmfamily,color=black]
    ...
    \node at (6,.1) {\Large ----- primary edge};
    \node at (6,-.9) {\Large - - - remote edge};
\end{tikzpicture}
\begin{center}
  You want to take a long bath
\end{center}
...
\vspace{-23mm}
\begin{adjustbox}{margin=1mm,frame,scale=.7}
