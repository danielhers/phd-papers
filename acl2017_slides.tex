...
\begin{tikzpicture}[level distance=15mm, sibling distance=13mm, ->,
  ...
  \node (ROOT) at (0,0) [circle] {}
    ...
    ;
  \draw[dashed,->] (graduation) to node [auto] {$A$} (Joe);
  \node at (6,-1) {predicted}
  \node (ROOT_) at (6,0) [circle] {}
    child {node (After_) {After} edge from parent node[left] {$L$}}
    ...
    child {node (moved) [circle] {}
    {
      child {node (Joe_) {Joe} edge from parent node[left] {$A$}}
      child {node {moved} edge from parent node[left] {$P$}}
      child {node [circle] {}
      {
        child {node {to} edge from parent node[left] {$R$}}
    ...
    ;
  \draw[dashed,->,red] (graduation_) to node [auto] {$C$} (Joe_);
\end{tikzpicture}
}
\end{frame}
