...
\title{Universal Conceptual Cognitive Annotation with Syntactic Parsing Systems}
...
\maketitle
\begin{abstract}
  Universal Conceptual Cognitive Annotation (UCCA) is a semantic grammatical scheme that assigns a complete structure to natural language text. We present the first attempt at UCCA parsing by converting between UCCA and formats usually used for syntactic annotation, and training syntactic parsers to produce UCCA graphs over the text.
\end{abstract}

\section{Introduction}

Universal Conceptual Cognitive Annotation (UCCA) is a recently introduced semantic annotation scheme. It takes a semantic approach to grammatical representation, describing relations between words and phrases in the text. The foundational layer can potentially be extended by any number of additional layers to provide refinements to the existing scheme, but it already covers many of the basic semantic relations in language. The annotation is represented as a directed acyclic graph (DAG), where edges denote semantic dependencies between abstract units. UCCA can be easily annotated manually without expert knowledge in linguistics, making it attractive as a resource for semantic tasks---an alternative to syntactic annotation, which is costly. A corpus containing 160K words from the English Wikipedia has been manually annotated.

This work is the first attempt at automatic UCCA parsing. Since there is no existing model to learn the general structure of UCCA DAGs, we use models originally designed for syntactic dependency parsing, and simply train them on converted UCCA data to produce UCCA parsers. Since the converted annotation is not as rich as the UCCA scheme, some information is lost and has to be recovered as part of the conversion.

\section{Results}

%\csvautotabular{/cs/nlp/danielh/workspace/ucca/results_latest.csv}
