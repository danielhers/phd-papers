...
Dependency schemes often apply some annotation convention that selects one of the sub-units
...
dependencies derived from the Enju parser\footnote{See \url{http://kmcs.nii.ac.jp/enju/}.},
...
\footnote{See Supplementary Material for the definition of $\mathrm{Label}$.}.
...
and further underscores the feasibility of transition-based 
...
To evaluate the relative importance of the different feature sets,
we perform feature ablation experiments on \textsc{bsp},
removing a set of features at a time,
and evaluating the resulting parser on the development set (\tabref{table:ablation}).
In the few cases where a feature set has only a small
or a negative contribution to the primary edge F-score, 
it has a positive contribution on the prediction of remote edges.
