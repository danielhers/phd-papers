...
\begin{abstract}
  ...
  %motivating research into   more formally expressive parsers.
...
\begin{figure}
  ...
  \begin{tikzpicture}[level distance=10mm, ->]
    ...
    \draw[bend left] (LKG) to[out=30, in=90, edge node={node [above] {\scriptsize $LA$}}] (moved);
  \end{tikzpicture}
  \\
  ...
  \end{tikzpicture}
\caption{\label{fig:ucca_example1}
  UCCA graphs for the sentences ``After graduation, John moved to Paris.'' and ``John decided to take a quick shower.''
  The first sentence demonstrates a linkage relation (the gray node and its outgoing edges),
  and the second sentence exhibits a discontinuous unit (``take a shower'').
  Both sentences have a remote edge (the dashed lines), where the node ``John'' has multiple parents.
}
\end{figure}
...
UCCA graphs for both examples are given in \figref{fig:ucca_example1}\footnote{UCCA graphs may contain nodes
  that do not correspond to a token in the text. Such units serve in UCCA to represent implicit relations
  and participants, such as the agent in passive sentences. Parsing these empty units
  is generally beyond the scope of this work.}.
...
CCG parsing \cite{ambati2015incremental}.
...
\paragraph{Notation.}
For a given set $L$ of possible edge labels,
a \textit{semantic constituency DAG} $G=(V,E, \ell)$ over a sequence of tokens $w_1, \ldots, w_n$
is a directed acyclic graph, where $\ell:E\to L$ is a function of \textit{edge labels}.
For each token $w_i$ ($i=1, \ldots, n$) there exists a leaf  $t_i \in V$.
{\it Terminals} are the leaf nodes in $V$.
...
and passages 701-965 as test set.
...
We have conducted additional experiments, not included in this work. We will briefly review their results:
(1) We applied the parsers to complete paragraphs, without splitting to sentences. MaltParser and the LSTM parser reached nearly the same scores as in the sentence experiments, but \textsc{uparse} and \textsc{bcs} failed to run with input above 500 tokens, for technical reasons.
