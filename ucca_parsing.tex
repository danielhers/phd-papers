...
\begin{abstract}
  ...
  yielding DAG (rather than tree) representations.
  ...
  (i.e., word-to-word) or abstract (i.e., not grounded at all in the words and phrases of the
  sentence) representations, this paper defines and tackles a constituency-based variant of the task.
  We experiment with the UCCA representation scheme --- the only broad-coverage constituency
  semantic representation ({\it BCSR}) scheme we are aware of with a sizable corpus --- exploring
  two transition-based approaches.
  First, we construct coverters from UCCA to two related formalisms, and use existing
  state-of-the-art parsers for these settings. Second, we construct a new transition-based
  parser for the task,
  building on recent advances in discontiguous constituency parsing.
  To our knowledge, this is the first work to tackle the broad-coverage constituency semantic
  parsing task.
