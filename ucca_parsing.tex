...
In order for a grounded\footnote{By \textit{grounded} we mean the text tokens are directly
annotated as part of the representation.}
semantic representation to cover the full range of semantic structures exhibited by
...
Another line of work addresses parsing into non-grounded semantic representation,
notably Abstract Meaning Representation (AMR)
...
(and imprecisely) detected.\footnote{Considerable technical effort has been invested in this task
with various approaches, including
\newcite{flanigan2014discriminative}, \newcite{pourdamghani2014aligning} and \newcite{pust2015parsing}.}
...
%and simplifies manual annotation \cite{abend2013universal}.
...
the result into AMR. They thus used a different set of transitions than
\textsc{bsp}, which parses strings directly into graphs.
Similarly, parsing in frameworks such as MRS \cite{copestake2005minimal} is based on
theories of syntax-semantics interface, which we avoid in this work and parse semantic structure
directly.

%techniques are not applicable here.
%bears on the representation scheme,
%where not all nodes correspond to words in the text and vice versa,
%and in parsing, as it requires 
%We note that not grounding the text within the semantic representation poses difficutlties,
%as it generally requires that this correspondence
%be automatically (and hence imprecisely) derived.
%AMR does not ground 
