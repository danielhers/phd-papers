...
{\scriptsize
  \begin{align*}
    \begin{aligned}
      M(G_P,G_G) = \\
      |\{(e_1,e_2) \in E_{P} \times E_{G} | yield(e_1) = yield(e_2) \& label(e_1)=label(e_2)\}| \\
    \end{aligned}
  \end{align*}
}
...
\textit{BCP} is our parser when trained on the whole data set, and \textit{BCP}$-R$ is our parser when trained without remote edges.
...
\oa{add paragraph-level scores, to show the reader we have done varied work. also possible linkage prediction scores and implicit prediction scores}
