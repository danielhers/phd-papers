...
We base our parser on existing techniques from transition-based parsing, with the addition of new features for parsing BSS and UCCA specifically.
...
%Third, we present the \textsc{bsp}, novel both in being the first broad-coverage DAG parser,
%as well as the first UCCA parser. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Background}\label{sec:ucca}
...
is a cross-linguistically applicable semantic representation schemes,
that builds on an established framework
...
UCCA takes a modular approach to semantics, representing each layer, or modules
of semantic distinctions, using a closed set of categories. Importantly,
the same set of categories can be applied to different lexical items, and across
languages. This stands in contrast to more lexically oriented schemes, such as FrameNet \cite{Baker:98},
PropBank \cite{Palmer:05} and AMR, which define an open-ended, fine-grained set of categories,
which while providing further information, is prone to coverage problems \cite{Palmer:10},
and results in divergent structures when applied to different languages \cite{Xue2014not}.

UCCA's foundational UCCA layer, which we use in this work, covers the predicate-argument
...
Formally, a UCCA structure is a DAG, whose leaves correspond to the words of the text. The
nodes of the graph, or its ``units'',
are either terminals or several elements (not necessarily contiguous) jointly viewed as a
single entity according to some semantic or cognitive consideration.
UCCA structures thus support all three criteria of BSS: multiple parents, non-terminal nodes,
and discontiguous units.
...
whose main relations are ``graduation'' and ``moved''. The participant ``John'' is a part of both Scenes,
while ``Paris'' only of the latter.
The two Scenes in this sentence are both arguments in the \textit{linkage relation} ``After'',
in this case expressing temporal linkage between them.
...
(e.g., complex adverbials, such a ``very clearly'')\footnote{
...
mainly addressing semantic role labeling of verbal argument structures,
...
and dependencies derived from the HPSG Enju parser\footnote{See \url{http://kmcs.nii.ac.jp/enju/}.},
and the Minimal Recursion Semantics representations of the Lingo ERG \cite{Flic:02}.
Like BSS parsing, SDP addresses a wide range of semantic phenomena,
and supports discontiguous units, and multiple parents. However, SDP does
...
Another line of work addressed parsing sentences into Abstract Meaning Representations (AMRs)
\cite{flanigan2014discriminative,vanderwende2015amr,pust2015parsing,wang2015transition,artzi2015broad}. 
While sharing much of this work's motivation, AMR parsing differs from the BSS parsing
task addressed here in a few ways. 
...
on parsing, as it requires that the alignment between words and logical symbols be automatically
...
the resulting tree into an AMR, and thus used a different set of transitions than
BSP, which parses strings directly into graphs.
...
and training an existing parser for that representation on the converted structures.
...
We convert BSS to constituency trees by removing a subset of the edges\footnote{Since
for a tree, labeling the nodes is equivalent to labeling the edges, we do not distinguish between the two options.
Note also that as the original structures may contain discontiguous units, the resulting trees may also contain discontiguous nodes.}.
Specifically, when converting UCCA structures, we note that UCCA marks one of the incoming edges
for each non-root as ``primary'' and the others as ``remote'' and ``linkage'' edges (see \figref{fig:graduation}).
...
We now turn to the direct approach for parsing BSS, a transition-based broad-coverage DAG parser.

In transition-based (also called \textit{shift-reduce}) parsing \cite{Nivre03anefficient},
...
During training, an oracle with access to the gold-standard annotated graph is used as a classifier.
...
and has been recently extended to discontiguous constituency parsing \cite{maier2015discontinuous},
...
\cite{sagae2008shift,tokgoz2015transition},
CCG parsing \cite{ambati2015incremental} and AMR parsing \cite{wang2015transition}.
Nevertheless, there are no constituency DAG parsers, a gap this work addresses.
Our parser combines techniques from discontiguous constituency parsing and dependency DAG parsing.
In addition, we introduce new UCCA-oriented features for parsing BSS.
We name our parser \textsc{bsp} (Broad-coverage Semantic Parser).

\paragraph{Transition Set.}
Given a sequence of tokens $w_1, \ldots, w_n$, we directly predict a BSS $G$ whose leaves
...
of the presented approach for BSS parsing. 
...
%The \textsc{bsp} obtains scores that are nearly identical to \newcite{maier2015discontinuous}
