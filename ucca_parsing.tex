...
Moreover, not embedding the text within the semantic representation,
often requiring that the alignment between words and logical symbols will be automatically
(and imprecisely) derived and potentially complicating the parsing task (see \secref{sec:realted}).

An alternative line of work tackles parsing into grounded semantic representations.
This line of work has focused on semantic dependencies, and has been explored systematically
in two recent Semeval shared tasks \cite{oepen2014semeval,oepen2015semeval}.
Bi-lexical dependencies are appealing as semantic representations, partly as they are
structurally and conceptually simple and lend themselves to strong and efficient
parsing methods. However, the assumption that each semantic unit has a unique head encounters
difficulties when applied to the representation of several frequent constructions,
such as coordination, prepositional phrases, and multi-word expressions.

In this work we define and tackle the constituency-based semantic parsing task.
Constituency representation is appealing, as it is both grounded in the words of the sentence,
and does not impose the limiting unique head assumption. Moreover, it still lends itself to
efficient and effective transition-based methods \cite{sagae_lavie:05,zhu:13,mayer:15}. 
We take a transition-based approach, motivated by their state-of-the-art performance on
related parsing tasks \cite{lstm_parser}, as well as their computational efficiency.

We follow two complementary approaches.
...
parser (see related discussion on the persaviness of non-projective structures in dependency
representations in \cite{}). Therefore, the conversion to these formalism and back will
lose some information. The details of the conversion and of the parsers can be found in
\secref{sec:conversion_approach}. 
Second, we present the first transition-based constituency parser that supports both discontiguous
constituents and multiple parents. We apply the parser both to an in-domain and out-of-domain
...
The contribution of this work is thus three-fold. First, we motivate and tackle the
constituency broad-coverage
...
\paragraph{Broad-coverge Semantic Parsing.}
...
Most work on sentence-level semantic parsing focused either on shallow methods, such as semantic
role labeling, yielding shallow predicate-argument
structures, or on highly restricted domains, such as parsing into factoid or database queries
\cite{geoquery,freebase}.
Recent years have seen much interest in 
broadly applicable tools for deep semantic parsing.
Two recent SemEval tasks have been devoted to a dependency variant of the task
\cite{oepen2014semeval,oepen2015semeval}, converting three major resources of semantic representation
into a common representation, namely the t-layer of the Prague Dependency Treebank, ... .
This line of work bears a close relation to our thing, and indeed we use their parsers, but
dependency representation entails several restricting assumptions that lead to difficulty for
general semantic representation.

A different line of work addressed AMR parsing \cite{flanigan2014discriminative,artzi2015broad}.

Finally, CCG-based stuff, like Boxer. 
