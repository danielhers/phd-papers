...
Research in statistical syntactic parsing has yieled an abundance of methods
for constructing a parser given annotated text, and has thus made enormous impact
on NLP. However, the formal constraints imposed by standard parsers, such as enforcing
a tree structure, which are arguably reasonable for describing the syntax of natural language
are clearly lacking when it comes to semantic parsing \cite{oepen2015semeval}, as they are
unable to capture argument sharing phenomena, such as control or coordination. Indeed,
all broad-coverage semantic schemes used in NLP we are aware of, allow multiple parents.

Most work on broad-coverage semantic parsing has focused on semantic role labeling, which
is confined to shallow argument structures, with specific emphasis on verbal structures.
The Abstract Meaning Representation (AMR) \cite{Banerescu:13}, which has attracted considerable
attention recently, provides a more complete semantic annotation in the form of
elaborate graphs, which represent the semantics of an entire sentence, without grounding
semantic sub-parts in the sentence's words and constituents.
Another important line of work that focuses on more elaborate structures, is that
of broad-coverage semantic dependency parsing, that seeks to represent 
all types of argument structures (including verbal, nominal and adjectival ones),
and the inter-relations between them. Three major approaches of this ilk have been
explored in two recent SemEval tasks \cite{oepen2014semeval,oepen2015semeval}: the Prague
Dependency Treebank tectogrammatical layer \cite{}, the Enju predicate-argument structures,
and dependencies derived from the Lingo ERG Minimal Recursion Semantics represenations \cite{}.

In this work, we propose the task of broad-coverage semantic {\it constituency} parsing. 
Constituency representation of semantics parsing has considerable advantages for semantic
representation, as it is able to represent constructions that have more than a single
head (e.g., coordination), to naturally represent cases where the identity of the head is disputed
(e.g., determiner-noun, preposition-noun etc.), and multi-word expressions (which are semantically
opaque and hence clearly have no head). \oa{mention Ivanovo et al., 2012}.

A parser for this setting must have support for: (1) multiple heads, (2) discontiguous units (which are
common even in the syntactic structures of some languages, and, as will be discussed, is important even
for English. 
  
\com{
  (3) It further requires support for discontiguous units (or non-projective parses), even in
  English, and other projective or mostly projective languages (e.g., extraposition, idiomatic
  discontiguous expressions, verb-particle constructions).
  (4) Recent work (notably, Semeval 14-task 8, Semeval 15-task 18) explored this broad-coverage
  semantic dependency parsing, compiling corpora from the PDT, Lingo ERG and ???.
  (5) 
  notably coordination, and leads to arbitrary parsing decisions).
  (6) there is currently no parser for the task.
  we propose one based on recent advances in transition-based parsing
  (both Wolfgang's and in NN embeddings). transition-based parsing is appealing for its
  conceptual simplicity, its high performance, and its speed.
  ...
  (9) our contribution: \#1 defining the constituency broad-coverage semantic parsing task
...
UCCA parsing is a novel task. Nevertheless, it resembles other parsing tasks in natural language processing. The techniques used in this work are inspired by some of them.
