...
\begin{figure}
  \begin{tikzpicture}[level distance=20mm, ->,
    ...
    \node (ROOT) [fill=black, circle] {}
      ...
      child {node (remote) [fill=black, circle] {}
      {
        child {node {to} edge from parent node[left] {\scriptsize $F$}}
        child {node (take) {take} edge from parent node[left] {\scriptsize $P$}}
        child {node {a} edge from parent node[right] {\scriptsize $F$}}
        child {node {quick} edge from parent node[right] {\scriptsize $D$}}
        child {node {shower} edge from parent node[right] {\scriptsize $P$}}
      } edge from parent node[left] {\scriptsize $A\quad$ \hspace{1mm} } }
      ;
    \draw[dashed] (take) -- (John) node[left] {\scriptsize $A$};
  \end{tikzpicture}
\caption{\label{fig:ucca_example_dag}
  UCCA graph for the sentence ``John decided to take a quick shower.''
  This sentence exhibits both a discontinuous unit (``take a shower'') and a remote edge (the dashed line).
  The node ``John'' has multiple parents.
}
\end{figure}
...
We convert constituency DAGs into constituency trees by removing a sub-set of the edges.
...
for each non-root as ``primary'' and the others as ``remote'' and ``linkage'' edges.\oa{should we
  say a word about which edges are primary and which are remote?}
...
to constituency trees using the above procedure, and then conver the result to dependency trees. 
...
with an edge labeled as $Terminal$ between them.
The parents of the pre-terminals are determined by the terminal's parent in the dependency
tree: if a dependency node $v$ is a child of the root in $T_d$, then $v$'s pre-terminal will also be a child of the root node. Otherwise, $v$'s pre-terminal is the child of the pre-terminal associated
with $v$'s head in $T_d$. Edge labels edges according to a function defined are defined heuristic rules, $\mathrm{Label}(u)$.\oa{this last sentence is unclear. what heuristics? why are they needed?}
%For example, in UCCA, if a node has a sibling that is annotated as a \textsc{ParallelScene}, we annotate it as a \textsc{ParallelScene} too.
...
i.e., an oracle that outputs a set of optimal transitions, based on the gold graph and
the current parser state\oa{unclear. optimal in what sense?}.
For example, the orcale would predict a \textsc{Node} transition if the stack 
...
and is taken\oa{what do you mean ``is taken''?} if it is a member of the oracle's set
of optimal transitions. Otherwise, a random optimal transition is taken and the
classifier's weights are updated.

\oa{much better. make sure that there aren't any other {\bf major} technical
...
named transition in \cite{maier2015discontinuous}.\oa{maybe also cite nivre's work that used SWAP for dep
  parsing?}
...
in the graph $G$. This novel feature\oa{is it novel? why do we need it then, where others didn't? or should
  they have used it too?} serves as a regularizer for the creation of new nodes.
...
\textsc{bcs} uses a different transition set, allowing for DAG parsing and UCCA-specific features.

Another parameter of variation is the classifier used by each of the parser.
...
Further experiments we have conducted, but did not include for shortage of space, are
