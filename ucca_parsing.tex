...
\begin{abstract}
  ...
  The paper discusses a set  structural properties, that should be supported
  ...
  (1) applying conversion procedures to the semantic structures,
...
e.g., \cite{maier2015discontinuous,wang2015transition}.
...
Second, we present a novel transition-based broad-coverage DAG parser,
Broad-Coverage Semantic Parsing (BSP; \secref{sec:direct_approach}),
based on extending existing transition-based parsing,
...
%  }
  ...
  \label{fig:gave}
  \end{subfigure}
  \caption{\small  \label{fig:examples}
    Semantic representation of the three structural properties
    required for BSS, according to the UCCA scheme. \figref{fig:graduation} includes a remote edge (dashed),
    resulting in ``John'' having two parents. % and a linkage relation (gray node and its outgoing edges).
    \figref{fig:home} includes a coordination construction (``John and Mary'').
    \figref{fig:gave} includes a discontiguous unit (``gave ... up'').
    Partial legend: $P$ -- a Scene's main relation, $A$ -- participant,
...
units is defered to future work, as it is likely to require different methods
...
on parsing, as it requries that the alignment between words and logical symbols be
