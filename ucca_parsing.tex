...
\daniel{Try another SotA constituency parser. Berkeley parser? Stanford parser is hard to train on a new tag set. Omri: don't think we need to do that. We focus on transition-based approaches.
  We might want to try another transition-based constituency parser, such as Zhu et al. 2013, but I don't
  think it's necessary, since Wolfgang's work extends that, doesn't it?}


\subsection{Conversion to dependency trees}

To evaluate dependency parsers in this scheme, we convert the data to dependency trees and back by Algorithms~\ref{lg:con2dep} and~\ref{alg:dep2con}.
...
Our parser architecture\oa{transition set? perhaps in this point it would make sense to define transition set vs. classifier, for the benefit of readers not versed in trans-based parsing.} is based on both transition-based constituency parsers \cite{zhu2013fast,maier2015discontinuous} and transition-based DAG dependency parsers \cite{sagae2008shift,tokgoz2015transition}.
