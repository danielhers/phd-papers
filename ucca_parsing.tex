...
\begin{algorithm}[t]
 \KwData{constituency tree $G$}
 \KwResult{dependency tree $T$}
 $V_T \leftarrow \emptyset$,
 $E_T \leftarrow \emptyset$\\
 \ForEach{$t \in \mathrm{TopologicalSort}(\mathrm{Terminals}(G))$}{
  $u \leftarrow \mathrm{Top}(t)$\\
  $p \leftarrow \mathrm{Parent}_G(u)$\\
  $t^\prime \leftarrow \mathrm{Head}(p)$\\
  $V_T \leftarrow V_T \cup \{t\}$\\
  $E_T \leftarrow E_T \cup \{(t^\prime, t)\}$\\
  $\ell_T (t^\prime, t) \leftarrow \ell_G(p, u)\}$\\
 }
...
\begin{algorithm}
 ...
 \KwResult{constituency tree $G$}
 $r \leftarrow \mathrm{Node()}$\\
 $V_G \leftarrow \{r\}$,
 $E_G \leftarrow \emptyset$\\
 \ForEach{$t \in V_T$}{
  $u \leftarrow \mathrm{Node()}$\\
  $V_G \leftarrow V_G \cup \{u, t\}$\\
  $E_G \leftarrow E_G \cup \{(u, t)\}$\\
  $\ell_G(u,t)\leftarrow\textsc{Terminal}$\\
  \eIf{$\ell_T(t) = \textsc{Root}$}{
   $E_G \leftarrow E_G \cup \{(r, u)\}$\\
   $\ell_G(r, u) \leftarrow \mathrm{Label}(u)$\\
  }{
   $p \leftarrow \mathrm{Parent}_G(\mathrm{Parent}_T(t))$\\
   $E_G \leftarrow E_G \cup \{(p, u)\}$\\
   $\ell_G(p, u) \leftarrow \mathrm{Label}(u)$\\
  }
 }
...
Our parser architecture follows previous work in transition-based constituency parsing \cite{zhu2013fast,maier2015discontinuous} and transition-based DAG dependency parsing \cite{sagae2008shift,tokgoz2015transition}.
...
\textsc{Node$_X$} creates a new node on the buffer as a parent of the first element on the stack, with edge tag $X$. \textsc{Implicit$_X$} does the same, but the created node is a child instead, and is marked as \textit{implicit}, representing it does not appear explicitly in the text.
\textsc{Left-Edge$_X$} and \textsc{Right-Edge$_X$} create a new edge between the first two elements on the stack, where the parent is the left or the right node, respectively. \textsc{Left-Remote$_X$} and \textsc{Right-Remote$_X$} do the same, but the created edge is marked as \textit{remote}, meaning the relation it represents is not explicit.
...
\textbf{\small Stack} & \textbf{\small Buffer} & \textbf{\small Nodes} & \multicolumn{1}{l}{\textbf{\small Edges}} & \multicolumn{1}{c|}{\textbf{\small Terminal?}} & \textbf{\small Stack} & \textbf{\small Buffer} & \textbf{\small Nodes} & \textbf{\small Edges} & \multicolumn{1}{c}{\textbf{\small Terminal?}} \\
$[r]$ & $w_{1:n}$ & $\{r\} \cup w_{1:n}$ & \multicolumn{1}{l}{$\emptyset$} & \multicolumn{1}{c|}{$-$} & $\emptyset$ & $\emptyset$ & $V$ & $E$ & \multicolumn{1}{c}{$+$} & \\
\multicolumn{5}{c|}{}\\
\hline
%\end{tabular}
%\end{adjustbox}
...
\textbf{\small Stack} & \textbf{\small Buffer} & \textbf{\small Nodes} & \textbf{\small Edges} & & \textbf{\small Stack} & \textbf{\small Buffer} & \textbf{\small Nodes} & \textbf{\small Edges} & \textbf{\small Terminal?} & \\
$S$ & $x \;|\; B$ & $V$ & $E$ & \textsc{Shift} & $S \;|\; x$ & $B$ & $V$ & $E$ & $-$ & \\
$S \;|\; x$ & $B$ & $V$ & $E$ & \textsc{Reduce} & $S$ & $B$ & $V$ & $E$ & $-$ & \\
$S \;|\; x$ & $B$ & $V$ & $E$ & \textsc{Node$_X$} & $S \;|\; x$ & $y \;|\; B$ & $V \cup \{ y \}$ & $E \cup \{ (y,x)_X \}$ & $-$ & $x \neq r$ \\
$S \;|\; x$ & $B$ & $V$ & $E$ & \textsc{Implicit$_X$} & $S \;|\; x$ & $y^* \;|\; B$ & $V \cup \{ y^* \}$ & $E \cup \{ (x,y^*)_X \}$ & $-$ & \\
$S \;|\; x,y$ & $B$ & $V$ & $E$ & \textsc{Left-Edge$_X$} & $S \;|\; x,y$ & $B$ & $V$ & $E \cup \{ (y,x)_X \}$ & $-$ & \\
$S \;|\; x,y$ & $B$ & $V$ & $E$ & \textsc{Right-Edge$_X$} & $S \;|\; x,y$ & $B$ & $V$ & $E \cup \{ (x,y)_X \}$ & $-$ & \\
$S \;|\; x,y$ & $B$ & $V$ & $E$ & \textsc{Left-Remote$_X$} & $S \;|\; x,y$ & $B$ & $V$ & $E \cup \{ (y,x)_X^* \}$ & $-$ & \\
...
\figref{fig:new_features} shows the features templates introduced in this work. $P$ stands for the number of parents a node currently has, and $C$ for the number of children.
...
\item NN-based classifers for the parser, dynamtic oracles and in general or the heavy machinary of transition-based methods.
\end{itemize}
