...
coordination structures (e.g., ``{\bf John and Mary} went home''; \figref{fig:non_terminal_nodes})
...
the phrasal verb ``gave ... up'' forms a single semantic unit (\figref{fig:gave}).
...
\begin{figure}
  ...
  \begin{tikzpicture}[level distance=10mm, ->]
    \node (ROOT) [fill=black, circle] {}
      ...
      ;
    \node (LKG) at (-2,0) [fill=black!20, circle] {};
%    \draw[bend left,dashed,->] (graduation) to node [auto] {\scriptsize $A$} (John);
%    \draw (LKG) to node [near start, left] {\scriptsize $$} (After);
%    \draw (LKG) to node [near start, left] {\scriptsize $$} (graduation);
%    \draw (LKG) to node [near start, right] {\scriptsize $$} (moved);
%=======
    \draw[dashed,->] (graduation) to node [auto] {\scriptsize $A$} (John);
    \draw[bend right] (LKG) to node [auto, left] {\scriptsize $LR$} (After);
    \draw[bend right] (LKG) to node [auto, below] {\scriptsize $LA$} (graduation);
    \draw[bend left] (LKG) to[out=30, in=90] node [above] {\scriptsize $LA$} (moved);
  \end{tikzpicture}
  }
  \\
  \scalebox{.9}{
...
\caption{\label{fig:ucca_example1}\small
  UCCA graphs for two example sentences:
  ``After graduation, John moved to Paris.'' and ``John decided to take a quick shower.''
  Both sentences have a remote edge (dashed),
  resulting in ``John'' having two parents.
  The first sentence includes a linkage relation (gray node and its outgoing edges)
  and the second sentence exhibits a discontinuous unit (``take a ... shower'').
