...
to be included in the model).\oa{are there any other important technical details?
  how does the oracle work? is there a learning rate? did you use beam search?
  these details sould be here, while the exact parameter settings should appear
  in the epxerimental setup section.}
...
\caption{\label{fig:transitions}
  The transition set of the BCS parser. Following standard practice,
...
While head word are not explicitly represented in the UCCA scheme, these
...
\end{minipage}
\end{adjustbox}
\caption{Feature templates for the BCS parser.\label{fig:features}}
\end{figure*}
...
We conduct our main experiments on the UCCA Wikipedia corpus, and use
the English part of the
UCCA \textit{Twenty Thousand Leagues Under the Sea} English-French parallel corpus as
out-of-domain data\footnote{Both can be found here \url{http://www.cs.huji.ac.il/~oabend/ucca.html}}.
\tabref{table:data} presents some statitics for the two corpora, demonstrating that while
the Wikipedia corpus is over ten times bigger, the overall statistics are
...
%Our conversion tools and direct parser are implemented in Python.
In all experiments, we use passages of indices not greater than 655
of the Wikipedia corpus as our training set, passages 656-700 as development set,
and passages 701-695 as test set.\oa{how many tokens per passage on average?}
While UCCA annotation applies can cross sentence boundaries, we adhere to the common
practice in semantic parsing and train our parsers on individual sentences,
discarding inter-relations between them.\oa{how many of the edges do we discard?}
In the out-of-domain experiments, we use the same parser, trained on the Wikipedia
corpus to the entire English \textit{Twenty Thousand Leagues Under the Sea} corpus as test set,
...
Assume $G_P=(V_P,E_p)$ and $G_G=(V_G,E_{G})$
are the predicted and gold-standard DAGs over the same
set of terminals $W = \{w_1,\ldots,w_n\}$, respectively.
For an edge $e=(p,c)$ in either the graphs,
where $p$ is the parent, and $c$ is the child, define its yield $yld(e) \subset W$ as the
set of terminals in $W$ which are descendants of $c$. We mark $e$'s label with $l(e)$, and
define:
...
\begin{multline*}
    M(G_P,G_G) = \\
    \left\vert\{(e_1,e_2) \in E_{P} \times E_{G}. 
    yld(e_1) = yld(e_2) \wedge l(e_1)=l(e_2)\right\vert
\end{multline*}
}

\vspace{-.5cm}

Labeled precision and recall are defined by dividing $M(G_P,G_G)$ with $|E_P|$ and $|E_G|$ respectively.
...
and one into CoNLL-style dependencies. In the first setting, we experiment with the {\sc UPARSE}
parser\oa{add url, citation}, and in the second with the MALT
parser\oa{add url, citation} and the LSTM parser \oa{add url, citation}.
...
We note that the LSTM parser uses a similar set of transitions to the
MALT parser, and only differs in terms of its classifier.\oa{is that true?}
...
In the BCS parser, we use a \textsc{MinUpdate}
value of 5\oa{explain what that is, what is the learning rate? are there
  any other hyper-parameters?}. We train BCS both where remote edges
...
BCS obtains comparable F-scores to the MALT and {\sc UPARSE} parsers
in terms of primary edges, but unlike them, is able to predict some
of the remote edges as well. 
Removing remote edges from the training data of BCS allows it to focus
...
results, supporting the robustness of the BCS results.

The LSTM parser obtains the best results, with a considerable margin.
Importantly, it obtains a 10.6\% higher F-score than the MALT parser,
which employs the same set of transitions.
This suggests that applying a similar LSTM-based approach to training BCS is
...
%The BCS parser obtains scores that are nearly identical to \newcite{maier2015discontinuous}
%and Nivre et al.~\shortcite{nivre2007maltparser} on regular edges, but also correctly
%predicts remote edges when trained on them.
%Training without linkage edges and implicit nodes improve the scores on regular edges, as a more simple model is easier to learn.

The conversion to constituency format gets almost a perfect score on the
regular edges, as it only omits extra linkage parents and implicit nodes.\oa{why does it not reach 100\% agreement on the regular edges?} The conversion to dependency format is loses considerably more information, since all of the non-terminal nodes are lost and have to be reconstructed by a simple rule-based inverse conversion.
...
\caption{\small
  ...
  and remote (right-hand side) edges. Top: results for {\sc UPARSE}
  ...
  Lower middle: results for our BCS parser, when trained on the complete UCCA DAGs (BCS),
  and when trained on UCCA trees, obtained by removing remote edges (BCS$_{Tree}$).
  Bottom: results for BCS and BCS$_{Tree}$ when tested on out-of-domain data.
...
technology to address the task, and by constructing a novel semantic parser, BCS, for
...
improvement by taking an LSTM-based approach to the local classifier of BCS.
...
domains, such as machine translation \cite{} and summarization{}. Introducting further candidate
representations into these inqueries is thus appealing and timely.

Future work will further explore improvements to the two presented approaches.
As our experiments suggest that an LSTM classifier is likely to substantially boost the parser
results of the BCS parser, we intend to explore this angle, as well as explore its applicability
