...
\begin{abstract}
  ...
  in both in-domain and out-of-domain settings, and in three languages.
...
\cite[SRL;][]{gildea2002automatic,Palmer:05,swayamdipta2017frame,Peng-EtAl:2018:NAACL},
...
Table~\ref{tab:id_results} shows our results on the English in-domain Wiki test set:
Again, MTL $F_1$ scores surpass single-task scores, and the best ones significantly so.
However, in this case, using all auxiliary tasks contributed less than just DM and UD$^{++}$,
the combination of which yielded the best scores yet in in-domain UCCA parsing,
with 74.9 $F_1$ on primary edges.

Finally, evaluating the full MTL model (\textit{All}) on the unlabeled auxiliary tasks yielded
...
These scores are not far from the state-of-the-art on these datasets
\cite{foland2017abstract,Peng-EtAl:2018:NAACL,dozat2016deep}.
...
These findings suggest the diversity of domains may have as important an effect on parsing performance, as the diversity of formalisms. Investigation of this matter will require a controlled experiments where the parser is trained on a large enough multiply-annotated parallel corpus, and is deferred to future work.
