...
Recently, \citet{Peng-EtAl:2018:NAACL} also	applied a similar approach to
...
We select the model from the epoch with the best average labeled $F_1$ score on the
UCCA development set in the respective language.
...
\caption{
Labeled precision, recall and $F_1$ (in~\%) for primary and remote edges,
on the \textbf{English Wiki} test set.
$\star$~indicates significantly better than \textit{Single}.}\label{tab:id_results}
\end{table}
...
13.7\% unlabeled Smatch $F_1$ \cite{cai2013smatch} on the AMR development set,
...
While we found the MTL models trained for UCCA parsing to perform poorly
...
when treated as a main task:
67.1\% Smatch $F_1$ on AMR
