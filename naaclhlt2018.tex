...
\begin{subfigure}[t]{0.5\textwidth}
  ...
  \end{tikzpicture}
  \captionof{figure}{Converted AMR graph.
  The text tokens have been added to the graph according to the alignments.
  Pre-terminal nodes and edges are included in order to show the node labels.
  The placeholder $\langle \ell \rangle$ corresponds to the terminal's lemma,
  $\langle v \rangle$ to its derivationally related verb form, and
  $\langle T \rangle$ to the terminal children's surface form.
  Numeric suffixes of \textit{op} relations have been removed,
  and names have been collapsed.}
...
All edges entering terminals bear the \textsc{Terminal} label.
...
and then simply attach the pre-terminals according to the original dependency edges.
