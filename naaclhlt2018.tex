  ...
  \begin{tikzpicture}[level distance=16mm, ->,
    ...
    \node (ROOT) [draw=black,ellipse] {move-01}
      child {node [draw=black,ellipse] {after}
      {
		    ...
		    {
		      child {node [word] {graduation \quad ,} edge from parent}
...
\begin{subfigure}[t]{0.5\textwidth}
  ...
  \end{tikzpicture}
  \captionof{figure}{Semantic dependencies (DM).}
...
Edges are labeled, and nodes are optionally too (to support AMR parsing).
