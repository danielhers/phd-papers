...
The scheme has proven useful for human machine translation evaluation \cite{birch2016hume},
but its applicability has so far been limited by the absence of a UCCA parser,
a gap this paper addresses.

Formally, a UCCA representation is a DAG whose leaves correspond to the tokens of
the text\oa{add footnote about we use sentences, but that it's annotated on paragraphs?}.
Nodes (or {\it units}) either correspond to a terminal or
...
that the parent represents. \figref{fig:examples} presents a few UCCA-anntoated examples.
...
Alongside recent progress in dependency parsing into projective trees and
similar methods\oa{what does ``and similar methods'' mean?} \cite{dyer2015transition,andor2016globally,kiperwasser2016simple},
...
A second property is \textbf{discontinuity} (similar to \textit{non-projectivity} in bilexical schemes).
E.g., in ``John \textit{gave} everything \textit{up}''
...
to UCCA represent units comprising more than one word.
...
We propose UCCP (Universal Conceptual Cognitive Parser), taking a transition-based approach.
We build on existing knowledge in tackling these three properties
in isolation, and define an extended set of transitions and features
...
We are further motivated by the remarkable performance of transition-based methods
...
We evaluate UCCP on the available English UCCA corpora, in both
...
between UCCA and bilexical graphs (\secref{sec:conversion}).
Results show superior results for UCCP, demonstrating the effectiveness of
...
adverbials such as ``very clearly'').\footnote{All UCCA-related resources can be found here:
...
We now turn to presenting UCCP (Broad-coverage Semantic Parser),
...
UCCP mostly builds on recent advances in discontinuous constituency
...
\caption{\label{fig:transitions}
  The transition set of UCCP. %Following standard practice,
...
We refer to the parser using this classifier simply as UCCP.
...
We refer to this parser as UCCP+embedding.
...
We refer to the neural network-based parser as UCCP+NN\footnote{For full implementation
...
\paragraph{Features.}
For the sparse perceptron-based parser (UCCP), we use binary indicator features representing
...
For the dense perceptron-based (UCCP+embedding) and the neural network-based parser (UCCP+NN),
...
embeddings to random 20-dimensional vectors. The embeddings for existing edge labels are
10-dimensional, and for parser actions are 5-dimensional.
...
\caption{\label{table:results}
  ...
  Bottom: results for our UCCP, trained on the original UCCA graphs.
}
\end{table*}
...
The sparse perceptron-based UCCP achieves comparable results to DAGParser on primary edges, but
on remote edges it is much more accurate, surpassing TurboParser by 10\% F-score.
The dense perceptron-based UCCP+embedding, using pre-trained word vectors, is less accurate on
...
Finally, the neural network-based UCCP+NN gets the highest scores in all measures, on both
the in-domain and the out-of-domain test sets.

\paragraph{Feature Ablation.}
To evaluate the relative impact of the different feature sets on UCCP,
...
edges, the direct approach does so much more accurately.
...
We also train the UCCP variants without remote edge transitions, yielding a tree parser.
...
UCCP $-$ \textsc{Remote} & 65.5 & 57.5 & 61.3 \\
UCCP+embedding $-$ \textsc{Remote} & 57.2 & 57.3 & 57.2 \\
UCCP+NN $-$ \textsc{Remote} &  \\
\end{tabular}
\caption{\label{fig:tree_approx_results}
  ...
  Bottom: results for our UCCP, trained on
...
with a considerable margin. Importantly, it obtains 9.2\% F-score higher than the MaltParser,
...
the parser, suggesting concrete paths for further improvements to UCCP:
...
the result into AMR, while our approach involves no such syntactic pre-processing.\oa{do we mention
  Shay's work?}
...
which is also the first grammarless parser that supports multiple parents, non-terminal
nodes and discontinuous units.
We compare the parser to a conversion-based parsing approach, demonstrating its advantage.
...
In addition to evaluating different neural network architectures for UCCP,
