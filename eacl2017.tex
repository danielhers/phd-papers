...
As these formalisms are more restrictive than UCCA, the conversion
...
A UCCA graph over a sequence of tokens $w_1, \ldots, w_n$ is a directed acyclic graph $G=(V,E, \ell)$,
...
In the conversion to dependency trees, we first convert UCCA
...
\paragraph{BSP.}
We train BSP for 16 iterations, using $\textsc{MinUpdate}=5$
...
Upper Bound & 93.5 & 83.5 & 88.2 & 77 & 46.8 & 58.2 \\
DAGParser &  &  &  &  &  &  \\
TurboParser &  &  &  &  &  &  \\
\hline
\multicolumn{4}{l}{\rule{0pt}{2ex} \footnotesize Direct Approach} \\
BSP & 64 & 55.6 & 59.5 & 16 & 11.6 & 13.4 \\
BSP+embeddings & 55 & 54.8 & 54.9 & 15.2 & 16.9 & 16 \\
BSP+NN &  &  &  &  &  &  \\
\hhline{=======}
\multicolumn{4}{l}{\rule{0pt}{2ex} \footnotesize Out-of-domain} \\
BSP & 60.6 & 53.9 & 57.1 & 20.2 & 10.3 & 13.6 \\
\end{tabular}
}
\caption{\label{table:results}
  ...
  Lower middle: results for our BSP, trained on the complete UCCA DAGs (BSP),
  and trained on UCCA trees, obtained by removing remote edges (BSP$_{\mathrm{Tree}}$).
  Bottom: results for BSP and BSP$_{\mathrm{Tree}}$ when tested on out-of-domain data (\textit{20K Leagues}).
  DAGParser is \newcite{ribeyre-villemontedelaclergerie-seddah:2014:SemEval} and
  TurboParser is \newcite{almeida-martins:2015:SemEval}.
}
\end{table}
...
BSP$_{\mathrm{Tree}}$ & 63.8 & 56.5 & 59.9 \\
\end{tabular}
