...
Finally, UCCA uses \textbf{non-terminal nodes} to UCCA represent units comprising more than one word.
...
%parsing results \cite{Schwartz:12}, while selecting the noun as the head may be
%more useful for information extraction.

No existing parser, to our knowledge, supports the three structural properties described above.
We propose the BSP UCCA parser\oa{we need to change its name to something with UCCA},
taking a transition-based approach. We build on existing knowledge in tackling these three properties
...
starting point for UCCA parsing, given the concepual similarity of UCCA's distinctions,
...
We evaluate BSP on the available English UCCA corpora, in both
...
Results show superior results for BSP, demonstrating the effectiveness of
...
%Comparison with tree approximation methods 


%while the distinctions made by UCCA are similar in spirit to those expressed by bilexical
%schemes (see \secref{sec:ucca}),
...
In addition to evaluating different neural network architectures for BSP,
