...
\oa{paragraph commented out. repetitive. see if some of the citations don't appear elsewhere}
\com{
...
and AMR parsing \cite{wang2015transition,wang-EtAl:2016:SemEval,dipendra2016neural,goodman2016noise}.
...
and applying dropout after each layer \cite{srivastava2014dropout}.\oa{do these things have hyperparameters we should mention? if so, in the exp setup}
...
In addition, we use novel, UCCA-specific features related to remote edges\footnote{See
  supplementary material for a full listing of the feature templates.}.\oa{we need to describe
  the UCCA features here because they are novel}.
...
For both the dense perceptron and the NN classifiers, we initialize the POS tag embeddings as random 20-dimensional vectors. The embeddings for existing\oa{what do you mean existing?} edge labels are 10-dimensional, and embeddings for parser actions are 5-dimensional.
...
As \parser{} is the first parser to support reentrancy, discontiguous units and
...
which we view as the closest existing match (they support two of the propoerties,
but not non-terminal nodes).
In order to assess the ability of existing parsers to address the task,
...
\parser{sparse} on primary edges, but on remote edges, \parser{sprase}
surpasses it considerably in performance
(DAGParser does not predict any remote edges in the out-of-domain setting).
TurboParser fares worse in this comaprison, despite somewhat better results on
remote edges.
...
Finally, using a feed-forward, \parser{NN} manages to exploit the embeddings,
and obtains substantially higher scores in all measures, both
in the in-domain and out-of-domain settings.
Its performance in absolute terms, of .. \%\oa{missing} F-score, is encouraging in light of the
inter-anntoator agreement, given UCCA’s inter-annotator agreement of 80--85\%
...
Performance on remote edges, which tree approximations miss entirely,
is of pivotal importance in this investigation, which focuses on extending the class of graphs supported by statistical parsers. Nevertheless, we believe that exploring tree approximation methods, thus building on the maturity of transition-based tree parsing, can inform the future development of DAG parsers in general and of UCCA parsers in particular.
...
To evaluate the relative impact of the different feature sets on \parser{sparse},
...
supplementary material for more detailed results.}
...
\textbf{unigrams} and \textbf{bigrams}\oa{these terms don't appear in the paper
  any more. explain what these are} features are especially
...
We present \parser{}, the first parser for UCCA, taking a transition-based approach.
Evaluated in both in-domain and out-of-domain settings, we show that coupled with a
feed-forward NN classifier, \parser{NN} surpasses bilexical DAG parsers on the task of UCCA parsing.
...
This paper makes a two-foled contribution to this literature. First, by proposing the first grammarless parser that supports multiple parents, non-terminal nodes and discontinuous units, and second, through detailed experiments with different architectures for the local classifier, and comparison both to bilexical approximations and to tree approximations.

Future work will delve deeper in the exploration of
conversion-based parsing approaches, including different target
representations and more sophisticated conversion procedures (e.g., \newcite{kong-15}),
