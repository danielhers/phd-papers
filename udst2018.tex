...
\begin{abstract}
  ...
  UD trees and graphs in a UCCA-like unified directed acyclic graph format,
...
We focus only on parsing, using a baseline model \cite[UDPipe;][]{udpipe,udpipe:2017}
...
provide richer information to the parser, and may improve parsing performance
...
\begin{figure*}[ht]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{LAS-F1 per test dataset.
...
We have presented TUPA, a general transition-based DAG parser, which was used
for the HUJI submission to the CoNLL 2018 shared task on parsing Universal Dependencies.
