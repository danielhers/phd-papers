...
(see examples in \S\ref{sec:format}),
...
\cprotect\caption{UD annotation adapted from \verb|reviews-341397-0003|,
...
so for example, the language-specific \verb|nsubj:pass|
...
We do the same for UD parsing, leaving elided predicates
and their associated dependencies to future work.
...
to the antecedent modified by the relative clause, also adding a \verb|ref|
dependency between the antecedent and the relative pronoun.
While these graphs may contain cycles, we remove them upon conversion to
the unified DAG format by deleting an arbitrary edge in each cycle, to
...
by 0.64 LAS-F1 points on the test treebank,
and 0.42 on the development treebank. 

As further ablation experiments, we tried removing POS features,
pre-trained word embeddings,
and remote edges (the latter corresponding to enhanced dependencies).
Removing the POS features does hurt performance substantially,
by 3.25 LAS-F1 points on the test set,
while removing the pre-trained word embeddings much less so,
even improving the performance on the development set
(in both basic and enhanced LAS-F1),
and the enhanced LAS-F1 on the test set too (by 0.3 enhanced LAS-F1 points).
Removing remote edges and transitions from TUPA makes almost no difference
to LAS-F1 (it degrades LAS-F1 by 0.28 on the test treebank but improves it
by the same number on the development treebank),
but of course enhanced dependencies
...
\hline
Original & 70.33 & 70.70 & 26.92 & 26.48 \\ 
$-$NER & 69.91 & 70.06 & 27.31 & 25.83 \\ 
$-$POS & 67.69 & 67.45 & 23.91 & 25.82 \\ 
$-$Embed. & 70.66 & 70.37 & 27.29 & 26.78 \\ 
