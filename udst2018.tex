...
\begin{figure}[t]
   ...
   \node[anchor=west] at (1.1,.275) {\small made};
   \draw[color=gray] (2.11,0) rectangle (5.1,.5);
   \node[anchor=west] at (2.11,.8) {$B$};
   \node[anchor=west] at (2.11,.26) {\small to feel very welcome .};
   \node[anchor=west] at (5.1,.8) {$G$};
   \node[fill=black, circle] at (6.35,.75) {}   
     child {node  {\tiny We} edge from parent [->] node[left] {\small nsubj}}
     child {node [fill=purple, circle] {}
     {
       child {node {\tiny were} edge from parent [->] node[right] {\small aux}}
...
Our system ranked 24th in the LAS~$F_1$ ranking
