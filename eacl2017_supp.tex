...
\]
For each such node $n$, we add $h^*(\mathrm{Parent}(n))$ as a head of $t$ in $A$.
...
\begin{algorithm}[h]
 ...
 \KwResult{set $A$ of bilexical arcs}
...
high-attaching.
Let $A=(V_d,E_d,\ell_d)$ be a dependency tree.
...
tree: if a dependency node $t$ is a child of the root in $A$, then $t$'s pre-terminal will also be a child of the root node. Otherwise, $t$'s pre-terminal is the child of the pre-terminal associated with $t$'s head in $A$. We add an intermediate node in between if $t$ has any dependents in $A$,
...
\begin{algorithm}[h]
 \KwData{dependency tree $A=(V_d,E_d,\ell_d)$}
 \KwResult{constituency tree $G=(V,E,\ell)$}
 ...
 \ForEach{$t \in \mathrm{TopologicalSort}(V_d)$} {
  ...
  \eIf{$t^\prime = \textsc{Root}$} {
   ...
   $p \leftarrow \mathrm{Parent}(t^\prime)$\;
   $E \leftarrow E \cup \{(p, u^\prime)\}$\;
   $\ell(p, u^\prime) \leftarrow \ell_d(t^\prime, t)$\;
  }
 }
...
We convert UCCA to constituency trees by removing a subset of the
...
leaving a tree formed by primary edges (see Figure 1a in the paper).
