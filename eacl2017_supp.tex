...
\section{Additional UCCA features and annotated examples}
...
\hline
nodes \\
\% implicit & 0.30 & 0.35 & 0.21 & 0.81 \\
\% linkage & 0.94 & 0.72 & 0.97 & 1.26 \\
\hline
edges \\
\% linkage & 2.99 & 2.25 & 2.99 & 3.33
\end{tabular}
...
The conversion from $G$ to a dependency graph consists of calculating a set $A$ of
labeled dependency arcs between the terminals of $G$.
...
\[
N(t) = \{n\in V \;|\; t=h^*(n) \wedge t \neq h^*(h^{-1}(n)) \}
\]
For each such node $n$, we add $h^*(h^{-1}(n))$ as a head of $t$ in $A$.
...
\begin{algorithm}[h]
 ...
 \ForEach{$u \in V$} {
  $h(u) \leftarrow \argmin_v \mathrm{Priority}(\ell(u,v))$\;
 }
 ...
 \ForEach{$t \in \mathrm{Terminals}(V)$} {
  \ForEach{$n \in N(t)$} {
   ...
   $A \leftarrow A \cup \{(t^\prime, t)\}$\;
   $\ell_d (t^\prime, t) \leftarrow \ell(u, n(t))\}$\;
  }
 }
...
Then, iterating over the terminals in topological order according to $A$,
...
graph: if a terminal $t$ is a child of the root in $A$, then $t$'s pre-terminal will also be a
child of the root node. Otherwise, $t$'s pre-terminal is the child of the pre-terminal
associated with $t$'s head in $A$.
...
\begin{algorithm}[h]
 ...
 \ForEach{$t \in \mathrm{TopologicalSort}(T, A)$} {
  ...
  $E \leftarrow E \cup \{(u, t)\}$\;
  $\ell(u,t)\leftarrow\mathit{Terminal}$\;
  $t^\prime \leftarrow \mathrm{Parent}_A(t)$\;
  \eIf{$t^\prime = \textsc{Root}$} {
   $E \leftarrow E \cup \{(r, u)\}$\;
   $\ell(r, u) \leftarrow \Label(r)$\;
  } {
   \eIf{$\exists v \in T : (t,v) \in A$} {
    $u^\prime \leftarrow \mathrm{Node()}$\;
    $E \leftarrow E \cup \{(u^\prime, u)\}$\;
    $\ell(u^\prime, u) \leftarrow \Label(u^\prime)$\;
   } {
    $u' \leftarrow u$\;
   }
   $p \leftarrow \mathrm{Parent}_G(t^\prime)$\;
   $E \leftarrow E \cup \{(p, u^\prime)\}$\;
   $\ell(p, u^\prime) \leftarrow \ell_A(t^\prime, t)$\;
  }
 }
  ...
  \KwResult{label $\ell \in L$}
   ...
   \uElseIf{$\exists v \in T : \ell(u,v) = \textit{ParallelScene}$}{
    \Return \textit{ParallelScene}\;
   }
   \uElseIf{$\exists v \in T : \ell(u,v) = \textit{Participant}$}{
...
\section{Tree approximation-based out-of-domain results}
...
\caption[]{\label{fig:features}
  Binary feature templates for BSP. Notation:
  \begin{itemize}
...
The first row corresponds to BSP as in the main experiment.
...
\hline
BSP & 62.6	& 55.7 & 58.9 & 20 & 12.9 & 15.7 \\
BSP$-$\textbf{unigrams} & 62.5 & 52.6 & 57.1 & 18.9 & 10.1 & 13.2 \\
BSP$-$\textbf{bigrams} & 59.8 & 50.0 & 54.4 & 18.2 & 12.2 & 14.6 \\
BSP$-$\textbf{trigrams} & 63.7 & 55.0 & 59.0 & 20.7 & 12.0 & 15.2 \\
BSP$-$\textbf{separator} & 62.9 & 53.5 & 57.8 & 17.8 & 11.7 & 14.1 \\
BSP$-$\textbf{extended} & 62.9 & 52.8 & 57.4 & 17.4 & 11.5 & 13.9 \\
BSP$-$\textbf{disco} & 63.6 & 53.6 & 58.2 & 19.7 & 11.5 & 14.5 \\
BSP$-$\textbf{counts} & 63.3 & 52.8 & 57.6 & 14.6 & 10.6 & 12.3 \\
BSP$-$\textbf{edges} & 63.5 & 54.9 & 58.9 & 23.6 & 14.5 & 17.9 \\
BSP$-$\textbf{history} & 63.1 & 53.2 & 57.8 & 23.7 & 14.5 & 18.0 \\
BSP$-$\textbf{remote} & 63.5 & 53.2 & 57.9 & 17.2 & 10.6 & 13.1 \\
BSP$-$\textbf{ratio} & 63.7 & 48.3 & 55.0 & 25.4 & 13.6 & 17.7
\end{tabular}
