...
and preservation of structure in translation \cite{sulem2015conceptual}.
...
\secref{sec:ucca} describes UCCA and its semantic and structural properties in more detail.
...
\secref{sec:related_work} summarizes related work, and \secref{sec:conclusion} concludes the paper.
...
thus forming a DAG (directed acyclic graph).
...
abstracts away from syntactic detail that does not affect meaning.\footnote{More precisely,
  UCCA abstracts away from syntactic variation that does not affect the aspects of meaning
  it models.}
For instnace, consider the following examples where the concept of a UCCA scene
has a different rationale from the syntactic concept of a clause.
First, non-verbal predicate are represented like verbal ones,
...
See \secref{sec:related_work} for a discussion of UCCA relative to other semantic schemes such
as AMR \cite{banarescu2013abstract}.
...
We propose \parser{} (Transition-based UCCA Parser), taking a
...
It surpasses both considerably in performance
...
The difference must be attributed to the classifier.

Using a feedforward NN and embeddings features,
\parser{MLP} obtains higher scores than \parser{Sparse},
but still does not manage to surpass the LSTM parser on primary edges.
...
\textsc{Remote} transitions, achieving a labeled
F-score on primary edges higher by up to 2\%,
but without remote edges.}
The bilexical DAG parsers are also quite limited in that respect,
due to the conversion error upper bound of 58.3\%.
However, the order-of-magnitude difference in favor of \parser{BiLSTM} suggests
the parser itself is a major factor.
