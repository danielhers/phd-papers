...
UCCA is distinct from other syntactic and semantic
schemes in terms of its content and formal structure.
...
but its applicability has been so far limited by the absence of a parser,
...
UCCA's \textit{foundational layer}, which we target in this paper, covers the predicate-argument
...
that distinguishes it from syntactic representations, mostly in cases where it
...
such as when they appear in copula clauses or in noun phrases. Indeed,
...
As such information is not explicit in the sentence's structure
...
No existing parser, to our knowledge, supports all these structural properties, required for UCCA
parsing.
...
% (Transition-based UCCA Parser).
Building on existing knowledge in tackling reentrancy, discontinuities and non-terminal nodes,
...
(``multi-layer perceptron''), using an architecture similar to that of \citet{chen2014fast},
...
(examples in \figref{fig:bilexical_example})
...
\oa{removed paragraph here; it's no longer necessary given the new results}
%Note that these conversion protocls result in a simplified task for the underlying parser.
...
This performance difference between them should therefore attributed to the classifier.
...
However, using better encoding of the input sentence,
...
\textsc{Remote} transitions, obtaining an increase of about 2\% of labeled F-score
points on primary edges, at the cost of not being able to predict remote edges. {\bf\color{red} OK?}}
...
Another line of work addresses parsing into AMR structures
...
and parsing jointly.
Our parser directly processes the input tokens,
and requires no concept identification.\oa{this last sentence may be misleading, because UCCA
  doesn't have such a term.}
...
We evaluate the proposed parser in both an in-domain and an out-of-domain scenario,
using several different transition-based approaches, outperforming a variety of
