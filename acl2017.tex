...
\begin{figure}[t]
    ...
    \node[main, fill=white!100] (mlp) at (4,5) {MLP};
	\foreach \i in {1,3} {
        \path (l\i) edge (mlp);
        \path (k\i) edge (mlp);
    }
    ...
    \node (transition) at (4,6) {Transition};
	...
	Vectors representation for the input tokens is computed
...
In \secref{sec:results} we report the upper bounds on the achievable scores due to the error resulting from the removal of non-terminal nodes\oa{the upper bounds for remote are really low; how likely is it that this explains the conversion-based parsers' miserable performance on remotes?}.
