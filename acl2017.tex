...
First, the semantic \textit{representations} are different. UCCA does not attempt to model
...
It creates non-terminal units as part of the parsing process, and does not require them
to be pre-calculated.
Furthermore, these parsers are AMR-specific, and support only a sub-class of graphs
that may be used in semantic parsing.
\parser{}, on the other hand, is theoretically able to achieve perfect accuracy on the UCCA corpus.
