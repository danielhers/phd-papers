...
which we view as the closest existing match (they support
reentrancy and discontinuity, but not non-terminal nodes).
...
To make the task as similar as possible,
we chose to keep the UCCA edge label set in the resulting bilexical graphs.
This inevitably incurs an error resulting from the removal of non-terminal nodes,
yielding an upper bounds on the achievable scores, reported in \secref{sec:results}.

We evaluate two parsers in the bilexical approximation setting:
DAGParser \cite{ribeyre-villemontedelaclergerie-seddah:2014:SemEval}, the leading transition-based parser that participated in the SDP task of SemEval 2014 \cite{oepen2014semeval},
...
%we believe that exploring tree approximation methods,
...
& \small Pred. nouns & \small Pred. adjectives & \small Expletive ``it'' \\
\hline
...
& 16 & 0 & 33.1 & 6.1 & 5.2 & 0 \\
TurboParser
& 28.6 & 0 & 34.6 & 8.4 & 8.1 & 0 \\
\textsc{uparse}
& 32.3 & 0 & 35.9 & 9.4 & 22 & 0 \\
MaltParser
& 41.7 & 0 & 35.5 & 5.6 & 6.4 & 0 \\
LSTM Parser
& 74.3 & 0 & 48.8 & 7.5 & 4.6 & 0 \\
\hline
