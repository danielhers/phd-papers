...
AMR parsing \cite{wang2015transition,wang-EtAl:2016:SemEval,dipendra2016neural,goodman2016noise,zhou2016amr,damonte2016incremental}
...
As a point of comparison, we assess the ability of existing bilexical graph
...
We use the Dynet package\footnote{\url{https://github.com/clab/dynet}} for implementing the NN classifiers. Unless otherwise noted, we use the default values provided by the package.
...
we perform fine-grained evaluation for \parser{} on various linguistic construction,
...
As \parser{} is the first parser to support UCCA's structure,
no direct comparison with an existing parser is possible.
We instead compare \parser{} to bilexical graph parsers,
...
training set.\footnote{See Appendix~\ref{appendix:conversion} for details about
the conversion.}
...
Upper bounds for the resulting scores are computed by applying
...
For completeness, and since tree parsers are generally much more developed than graph parsers,
we also perform a tree approximation experiment, converting UCCA to (bilexical) trees
and evaluating tree parsers on them.
In our setting, the conversion to trees consists simply of removing remote edges from the graph.
Note that this results in a simplified task for the underlying parser,
and furthermore, that performance on remote edges is of pivotal importance in this investigation,
which focuses on extending the class of graphs supported by statistical parsers.
Nevertheless, we believe that exploring tree approximation methods,
thus building on the maturity and cumulative experience with transition-based tree parsing,
can inform the future development of graph parsers in general and of UCCA parsers in particular.

We explore two conversion scenarios: one into (possibly discontinuous) constituency trees,
and one into (possibly non-projective) bilexical dependency trees.
In the first setting we experiment with \textsc{uparse}
...
\textsc{ArcEager} transition set and the SVM classifier\footnote{Preliminary
experiments with other configurations of MaltParser yielded lower scores and
...
& 94.5 & 87.7 & 91 & 77.3 & 46.8 & 58.3
& 94.8 & 88 & 91.3 & 66.3 & 32.3 & 43.4 \vspace{.1cm} \\
DAGParser
...
(specifically, LSTMs and GRUs) in NLP tasks.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
& Aspectual verbs & Light verbs & MWE & Pred. nouns & Pred. adjectives & Expletive ``it'' \\
\hline
\#edges & 17 & 45 & 721 & 49 & 32 & 15 \\
\hline
\multicolumn{7}{l}{\rule{0pt}{2ex} \footnotesize Parser F-scores (\%)} \\
\parser{Sparse} & 26.7 & 0 & 42.5 & 10.5 & 44.8 & 0 \\
\parser{Dense}  \\
\parser{MLP} \\
\parser{BiLSTM}
\end{tabular}
\caption{\label{table:constructions_results}
Results of evaluation on UCCA edges corresponding to specific linguistic constructions in the Wiki test-set.}
\end{table*}
...
\citet{wang2015transition}, \citet{wang-xue-pradhan:2015:ACL-IJCNLP} and \citet{wang-EtAl:2016:SemEval}
applied a transition-based approach to AMR parsing, and \citet{goodman2016noise} improved upon
their parser using imitation-learning algorithms.
Their method involved first syntactically parsing the input, and then converting
the result into AMR, while our parser directly processes the input tokens.
...
\appendix
\section{Conversion Protocols for the Bilexical Graph and Tree Approximations}
