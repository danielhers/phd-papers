...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[level distance=12mm, ->,
    ...
    \node (ROOT) [circle] {}
      ...
      child {node {'s} edge from parent node[left] {\scriptsize F}}
...
predicate nouns and adjectives are represented like verbal ones,
even when not expressed by a copula clause.
For example, in \figref{fig:graduation}, ``graduation'' evokes a scene,
despite lacking a verb.
Furthermore, ``John'' is marked as a Participant in this
...
which only sometimes actually express semantic possession.
In \figref{fig:home}, ``trip'' evokes a scene in which ``John and Mary'' is
a participant, despite being linked with a possessive construction thereto.
Some of these semantic distinctions are missing from other schemes, e.g. AMR
...
\parser{BiLSTM}
& \textbf{74.4} & \textbf{72.7} & \textbf{73.5} & \textbf{47.4} & \textbf{51.6} & \textbf{49.4}
& \textbf{68.7} & \textbf{68.5} & \textbf{68.6} & \textbf{38.6} & \textbf{18.8} & \textbf{25.3}
\end{tabular}
...
\section{Bilexical Graph and Tree Conversion}
...
The corresponding bilexical tree is given in \figref{fig:dep_example}.
The inverse conversion is simply the identity function, as every tree is also a graph.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[t]{\columnwidth}
	...
	\depedge{5}{7}{A}
	\end{dependency}
  \caption{Bilexical tree approximation for the same graph.}
