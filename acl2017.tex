...
the presented approach.\footnote{Our code and models will be made freely available upon publication.}
...
\caption{Bilexical graph approximation for sentences in \figref{fig:examples}.}
...
Furthermore, existing transition-based AMR parsers
are not general DAG parsers, but are
only able to predict a subset of reentrancies and discontinuities
(for example, \citet{damonte-17} pop the dependent in LArc transitions\daniel{clarify}).
They are thus limited to a sub-class of AMRs in particular,
and of graphs that may be used in semantic parsing in general.
\parser{}'s transition set, on the other hand, allows general DAG parsing\daniel{prove}.
