...
\title{Syntactic or Semantic Parsing---Which is Harder?}
...
Parsing into semantic meaning representation is generally seen as a harder task than
syntactic parsing, as the current state-of-the-art scores attest: in English in-domain parsing,
\citet{} achieve ... F1 when parsing the Wall-Street Journal into Penn Treebank syntactic constituency trees,
and \citet{} achieve ... LAS when parsing it into Stanford syntactic dependencies.
On the other hand, \citet{} achieve only ... F1 when parsing the same corpus into semantic dependencies
with the ... target representations, respectively,
and \citet{} achieve only ... Smatch F1 in AMR parsing on the same corpus.

In this work we attempt to explain the difference in scores between the two sets of tasks.
Apart from the obvious difference in the actual task with the distinctions it requires a parser to make,
we investigate other factors that may contribute to this gap.
We take Universal Dependencies as a representative of the syntactic side,
and UCCA as a semantic structure representation scheme.
On the English EWT corpus, the current state-of-the-art in UD parsing is ... LAS F1 by \citet{},
whereas \citet{} achieve at most ... F1 in UCCA parsing on the Wiki corpus.
...
\begin{figure}[!ht]
  \centering
    ...
    \end{tikzpicture}
\caption{\label{fig:conjoined_verbs}}
\end{figure}
...
Univesal Dependency Parsing \cite{udst:overview2018},
