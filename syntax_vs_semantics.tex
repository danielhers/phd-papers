...
\title{Is Syntactic Parsing More Accurate than Semantic Parsing?}
...
\begin{abstract}
  Syntactic parsers are known to outperform semantic ones in absolute terms,
  using similar evaluation metrics, but the sources of this gap have never
  been empirically clarified.  
  We explore the fine differences in the evaluation of syntactic
  and semantic parsing, taking Universal Dependencies (UD) and 
  Universal Conceptual Cognitive Annotation (UCCA) as a test case.
  Our findings suggest that the performance gap can be partially explained
  by the different set of tokens used by the two tasks, and by the tendency
  of UD structures to underspecify some fundamental semantic distinctions.
  When comparing parsing performance in more similar terms, the gap in performance
  is of only ...\%.
\end{abstract}
...
Parsing into semantic representations is generally seen as a harder task than
syntactic parsing. Empirically, syntactic parsers outperform semantic parsers
by 10-20 points, despite using conceptually similar measures, that measure
the number of overlapping edges between the gold standard and predicted structures. 
Indeed, in English, in-domain parsing reaches 95.13 F1 when constituency-parsing
the Penn Treebank \citep{P18-1249}, and 93.76 Labeled Attachment Score (LAS)
when parsing it into Stanford syntactic dependencies \citep{dozat2016deep},
the best reported result in AMR (LDC2016E25 test set) is 74.4 Smatch F1 \citep{lyu2018amr},
and the best for UCCA parsing in English is 74.9 F1 \citep{hershcovich2018multi}.
The only exception to this performance gap between syntactic and semantic parsing we
are aware of is \citep{P18-2077},
who reported 93.7 labeled F1 for the DM target representations on English \cite{oepen2016towards}.

This work sheds light on the reasons for this performance gap, by considering the performance
of parsers on different constructions in isolation.
We take Universal Dependencies \cite[UD; ][]{nivre2016universal} as a representative
of the syntactic side, and Universal Conceptual Cognitive Annotation
\cite[UCCA; ][]{abend2013universal} as an instance of a semantic structure representation scheme.

In this case, the gap is less major
...
whereas \citet{hershcovich2018multitask} obtains up to 74.9 primary labeled F1
in in-domain UCCA parsing on the Wiki corpus \cite{hershcovich2017a}.
We note that UD parsing performance on English is roughly 85 LAS, whereas performance
with English-specific dependency schemes are usually 90 LAS or higher. This difference
stems from the more abstract nature of UD.\oa{do we attempt to explain this?}
