...
Parsers supporting the combination of these criteria are scarce,
and rely on syntactic theories of compositional semantics or avoid grounding semantic units
(\secref{sec:related_work}).
...
structure, is similar to distinctions conveyed by dependency schemes.
...
As opposed to our direct approach,
their method involved first syntactically parsing the input, and then converting
the result into AMR.


\paragraph{Grammar-Based Methods.}
Linguistically expressive grammars such as HPSG, CCG and TAG
provide a theory of the syntax-semantics interface that has been used to build semantic parsers
by defining compositional semantics on top of them \cite{Hock:03b,Clar:03}.
Depending on the grammar and the implementation, these semantic parsers can support
some or all of the BSS criteria.
Nevertheless, this line of work differs from our discriminative grammarless approach,
which uses dependency-like semantic structures,
making less commitments as to the relation between syntax and semantics.
This both changes the parsing task and the type of semantic distinctions used.
...
leaving a tree structure formed by the primary edges (see \figref{fig:graduation}).
...
Supplementary Material for feature ablation results.}
...
We explored a conversion-based parsing approach to assess the ability of existing
technology to address the task,
and make a further contribution by first experimenting on UCCA parsing.
Results show that UCCA can be parsed with 69.2\% primary F-score,
and suggest means for improvement by applying an NN-based classifier to \textsc{bsp}.
